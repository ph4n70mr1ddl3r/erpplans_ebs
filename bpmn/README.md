...
| BPMN processes | 5,455 (one per workflow block, incl. the 23 `###`/`####` sub-workflows such as W5A) |
| Tasks | 23,050 |
| Sequence flows | 28,505 (= tasks + one task→end flow per process) |
| Diagrams (DI) | 5,455 — every process carries full BPMNDiagram/BPMNPlane/shape/edge interchange |
...
- Control flow is rendered as the **documented sequence**; conditional logic embedded in step
  ...
  (5,455 judgment calls), not something to fake mechanically.
...
*Generated by `07-methodology/generate-bpmn.py` · source corpus: 5,455 workflow blocks across 188 value streams / 569 process areas*
