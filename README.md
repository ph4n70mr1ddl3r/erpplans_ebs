...
│   ├── workflows/                      5,433 workflows organized by value stream
...
│   │   ├── workflow-criticality-classification.md  Tier 1/2/3 confirmed priorities (5,456 rows)
...
│   │   ├── VS-16-order-to-cash/                 32 workflows (3 process areas)
...
│   │   ├── VS-25-esg-sustainability/            32 workflows (3 process areas)
...
│   │   ├── VS-113-enterprise-architecture-application-portfolio-and-technology-strategy/ 32 workflows (3 process areas)
...
│   ├── optimal-table-of-organization.md  OFFICIAL table of organization of record — THE ACTUAL STRUCTURE (promoted 2026-09-14; gap-filled 2026-09-18; Trade dept disabled—prepared 2026-09-23 (x); IT's TPS build squad deferred—prepared 2026-09-23 (ad) and OMO build squad deferred—prepared 2026-09-23 (ah) — active HQ 511 / total 6,911; role-level workflow-anchoring contract 2026-09-23): design HQ 532 / total 6,932, every HQ role defined in §5.3 (pre-optimization 362 / 6,762 retained as the §5.1 reference column; official elevation 2026-09-09; two-tier IT sourcing + agentic-AI model)
...
│   ├── weak-anchor-demand-verification.md  GENERATED weak-anchor watchlist demand verification (batch 38; the 1–2-workflow roles measured against virtual-gemba per-role annual demand — CONFIRMED / OVERLOAD / UNDER-UTILIZED / EXERCISED-THROUGH / MEASURED — CAPACITY UNMAPPED / ZERO-DURATION / UNPARSEABLE-FREQ / NO PARSED CADENCE verdicts; regenerate after any anchoring change)
│   ├── role-demand-verification.md  GENERATED universal role-demand register (batch 52c, 2026-09-26) — ALL 259 chartered roles measured against chartered capacity per universal-role-demand-verification-spec.md (verdict + residual + state + confidence columns, department roll-up; engine-parity parts attribution with the grain-guarded alias-surface fallback — substring claiming removed at 52c, EXERCISED-THROUGH restored; regenerate via 07-methodology/verify-role-demand.py — unpinned until Phase 3, do not hand-edit)
│   ├── role-demand-disposition-queue.md  GENERATED P2 disposition queue (batch 54, 2026-09-26) — the 79 active-OVERLOAD rows with per-cell evidence, auto-classified SCALE-ARTIFACT / CELL-AUDIT / FIELD-SIGNAL / DISTRIBUTED with the engine grammar-gap sensitivity marked (PER-UNIT? / PERIOD?), plus the charter naming worklist ranked by unnamed hours (joined at the register's demand basis since batch 54; regenerate via 07-methodology/triage-role-demand.py — do not hand-edit)
...
│   ├── policies/                     ← Corporate Policy Manual — the 75 governing policies (POL-G/F/P/H/I/D/R/S/B) the workflows presume, mapped to owners, approvers & review cycles per W1729 (README.md = framework + master register + coverage-decision gap analysis; 9 domain manuals: governance incl. AML/competition/IP/ERM/board/government-sales, finance incl. property/land-use & lending, procurement, HR incl. labor relations/CBA, IT security incl. OT, privacy/records, retail ops incl. recall/gift cards/advertising/ticketing, HSE incl. live goods/security/stewardship, continuity)
...
│   ├── fit-gap-analysis.md            109-row capability disposition register (85 standard / 2 PER / 1 EXT / 1 LOC / 5 INT / 15 BUILD incl. 5 already-built platforms / 0 EDGE / 0 OPEN), EBS-held vs payroll-built statutory split, two-tier resolution record + 2026-09-15/16 EBS-exhaustion audits (eight passes) + 2026-09-21 EBS documentation-coverage closures (batch 26–27, all 25 EDC rows) + the 2026-09-22 Vision-findings re-disposition (VF-2 H11 → BUILD; VF-1 A11 naming), standard-first KPIs (78.0%)
...
│   ├── licensing-bom.md               License bill of materials — EBS perpetual + support vs Fusion Cloud scenarios, from the module footprint & user/volume drivers (negotiation baseline; totals re-derived by audit-model-docs) — carries the 2026-09-23 GPL-sweep entitlement flags (Asset Tracking L11496, Ship-and-Debit L72211, QP EOL L31659, Product Hub Add-on L42175; no totals moved)
...
│   ├── ebs-oratest-verification.md    Capability verification of the documentation set against the operating company's R12.2.4 PROD-clone test rig (the second installation of record) — FP footprint re-mapped onto the rig registry, the ten-path transactional matrix re-executed 9 GREEN + 1 PARTIAL with both Vision instance-side blocks GREEN on the rig, the two paths without rig evidence (HR hire; FA mass additions) executed end-to-end for the report, findings OR-1–OR-6
│   ├── ebs-documentation-coverage.md  Inward-direction reconciliation of Oracle's own R12.2 documentation library (376 guides / 133,782 pages in ebs_docs/) against this model — 165 business products triaged, 90 already adjudicated, 50 recorded not-needed with reasons, 25 EDC gap rows all closed (batch 26–27); confirms Financials for Asia/Pacific carries no Philippine content; §7 (2026-09-23) sweeps the Component GPL itself — all 307 priced part numbers dispositioned, 0 unexamined
...
└── 07-methodology/              ← Technical system architecture and guidelines
    ...
    ├── ai-first-operating-guide.md   The AI-first operating guide — ERP-agnostic doctrine (ten laws, six layers, capability contracts), sourcing neutrality, autonomy ladder, agent factory, the EKB as persistent memory + consistency engine, the no-confusion decision system, and the seven-phase adoption path (v1.27)
    ├── validate-repo.sh              Cross-reference validation (86 checks)
    ...
    ├── manhours-rollup.py            Living man-hours derivation from every Steps table's Role × Duration columns (classification-honest: effort/per-unit/elapsed/automated/cadence-only buckets; splits live-capability vs dormant designed-capacity effort from dormant-capability-register.json — the registry DISABLED — rows plus the sourcing-register TPS/OMO squad-deferral estates; reads the tree, never edits it)
    ...
    ├── final-semantic-coverage.py    Final full-coverage semantic pass (review #71 — detector suite over the 3,122 unaudited workflows; closed the sampling loop to 5,363/5,363; the post-closure transition runs extended the registry through 5,370/5,370; the 2026-09-23 seventy-fourth-wave review admitted all 62 remaining post-closure workflows W5518–W5579 in one pass via the documented transition path — full read of every block + the D1–D4 suite + the five D2 citation hits adjudicated into KNOWN_GOOD (RA 10175/RA 7279/RA 10821), with four in-read defects repaired — the registry now closes at 5,432/5,432 through W5579 (W5580 — batch 30 — rides the documented transition path for its semantic read))
    ...
    ├── fix-store-role-scope.py     Fifty-sixth-wave store-layer role-scope true-up — 93 structured-cell edits: store CS Rep title form (Owner/Participants/Role cells + prose, 9 store files), Floor Associate → Sales Associate (14 cells), W562 LP daily routine → Store Manager (9 cells), 4 dual-scope Frequency fields re-scoped (forms guarded by audit-model-docs store_scope_hits)
    ├── fix-role-anchor-worklist.py  Role-anchoring worklist remediation (batch 31, 2026-09-23) — cleared the 56-role zero-anchor worklist: 49 Participants-line anchors across 33 PA files (42 register roles, both IAP seats, 9 DC-roster roles into their mandate workflows), picker/packer/integration-specialist/payroll-accountant/marketing-data-analyst/assistant-controller vocabulary promoted to the chartered titles it denotes, 3 §7.2 staffing-note fragments re-scoped out of the charter census; census now 259/259/0 with anchor_zero=0 pinned by Check 71
    ├── fix-owner-grain-promotions.py  Owner-grain promotion sweep (batch 33, 2026-09-23) — first remediation wave off the workflow→role gap analysis: Logistics Manager → Fleet & Logistics Manager (30), Data Protection Officer (DPO) → Data Privacy Officer (DPO) (26), the costing/fixed-asset register decision (GL & Consolidation mandate re-scope, HC-neutral; Cost Accountant / Fixed Asset Accountant / Cost Accounting Manager promoted, 36), PA-48.2 retail-media owners → Retail Media & Marketplace Manager; Class B 1,612 → 1,519, Class C 3,610 → 3,722, Class A 211 → 192
    ├── fix-owner-grain-promotions-2.py  Owner-grain promotion sweep II (batch 34, 2026-09-23) — FP&A → FP&A Manager (25), Compliance Officer → Compliance Manager / MLRO (22), Market Research Analyst → Insights Analyst (16); Class B 1,519 → 1,447, Class C 3,722 → 3,799, Class A 192 → 187
    ├── fix-owner-grain-promotions-3.py  Class-A cluster promotion sweep (batch 35, 2026-09-23) — EEO → Energy Manager (~13, VS-120 RA-11285), Event Marketing Manager → Promotions & Campaigns Manager (~9, VS-139), Trade Capability → Learning & Development Manager (~9, VS-123), Store HR Administrator → Store HR Coordinator, Dark Store Shift Supervisor → DC:Shift Supervisors; gap analyzer now annotates system-owned Class-A workflows as by-design; Class B 1,447, Class C 3,871, Class A 150 (17 estate + 3 automated, 130 open)
    ├── anchor-step-level-wave1.py  Step-level anchoring wave 1 (batch 39, 2026-09-23) — nine weak-anchor roles elevated into Role (R) cells of their mandate steps (Payroll Supervisor, Payroll & Statutory Remittance Officer, Timekeeping & Attendance Analyst, Senior Demand Planner, Purchasing / PO Specialist, Tax Compliance & eFPS Specialist, TA Coordinator, Ecommerce Support Specialist, Retail Media Operations Specialist); weak 62 → 59, the 56-role NO-PARSE list is the named step-level anchoring backlog
    ├── anchor-step-level-wave2.py  Step-level anchoring wave 2 (batch 41, 2026-09-23) — ten further NO-PARSE weak-anchor roles elevated (Technical Accounting Manager, Accounting Policy Analyst, Contracts & Commercial Manager, Audit Manager, Field Compliance Auditor, Operations Compliance Lead, Forensic / Fraud Investigator, Compensation Analyst, Strategy Analyst, Field Communications Manager); weak 59 → 58
    ├── anchor-step-level-wave5.py  Step-level anchoring wave 5 (batch 47, 2026-09-23) — Senior FP&A Analyst (capex validation), Quality & Workforce Analyst (Tier-2 resolution QA — re-cut by the batch-51 dispositions; the QWA mandate is on the anchoring backlog), Sourcing & Screening Coordinator (pre-hire checks), Legal Counsel — Contracts (contract negotiation), Privacy Officer (consent governance — re-cut by the batch-51 dispositions; the DPO's step-3 quarterly audit stands), Promotions Specialist + Promotions & Vendor-Funding Coordinator (markdown ops); weak 56 → 53; verification reads 2 CONFIRMED / 2 OVERLOAD / 3 UNDER-UTILIZED / 46 NO PARSED CADENCE — superseded by the batch-51 dispositions (the verification now reads OVERLOAD 0)
    ├── anchor-step-level-wave6.py  Step-level anchoring wave 6 (batch 48, 2026-09-23) — DC-floor elevations: Discrepancy Analysts (GR/IR discrepancy resolution, PA-04.1) + Cross-Dock Team (labor allocation, PA-04.3); verification reads 2 CONFIRMED / 2 OVERLOAD / 4 UNDER-UTILIZED / 45 NO PARSED CADENCE
    ├── anchor-step-level-wave7.py  Step-level anchoring wave 7 (batch 50, 2026-09-25) — twelve participant-only weak-anchor roles elevated into mandate-exact Role (R) cells (DC field seats, ecommerce/FM/IT seats; Facilities/Utility deferred — slash-compound charter form); verification reads 5 CONFIRMED / 4 OVERLOAD / 16 UNDER / 2 EXERCISED / 2 MEASURED-UNMAPPED / 20 ZERO-DURATION / 4 NO PARSED — new capacity-real candidate: Special Handling Lead 178% — superseded by the batch-51 dispositions (the putaway cell re-cut to W3 step 9a's per-occurrence slice; the lead now reads ZERO-DURATION and the verification OVERLOAD 0)
    ├── anchor-step-level-wave3.py  Step-level anchoring wave 3 (batch 44, 2026-09-23) — Banking & Cash-Management Specialist (petty-cash custodian control, PA-18.2), Corporate Secretary Analyst (disclosure controls, PA-173.2), Import Documentation Specialist (shipping documents, PA-02.2), B2B Support Representative (wholesale SO entry, PA-11.3), Replenishment & Allocation Analyst (end-of-cycle confirmation, PA-07.3), CPM Analyst (corporate scorecard, PA-33.2); weak 58 → 56, two roles now CONFIRMED
    ├── anchor-step-level-wave4.py  Step-level anchoring wave 4 (batch 45, 2026-09-23) — Real-Estate & Site-Selection Analyst (PA-20.1 ×2), Maintenance & Projects Coordinator (PA-138.1), Sourcing & Screening Coordinator (PA-121.2), 3PL & Freight Specialist (PA-110.1), Senior LP Analytics Analyst (PA-23.1 — anchor re-cut by the batch-51 dispositions to W710 step 2's weekly shrinkage analytics), Marketing Comms Specialist (PA-14.3), ESG Reporting & Data Analyst (PA-25.3), Wellness Coordinator (PA-83.3) elevated; weak 56 → 54; verification surfaces Senior LP Analytics Analyst at 1,014% utilization (capacity-decision candidate — superseded by the batch-51 dispositions; the Senior now reads UNDER-UTILIZED)
    ├── verify-weak-anchor-demand.py  Weak-anchor demand verification generator (batch 43; re-derived batches 49–51, 2026-09-25 — batch 49 fixes the plural-unit duration defect in the motion engine's parse_minutes ('2 hours'/'4 hrs'/'24 hours/year' had silently measured 0 minutes across 3,775 corpus cells) and the dump parser's silent drop of thousands-separated utilizations, and adds the EXERCISED-THROUGH verdict measuring weak-anchor roles whose named cells attribute to the broader seats their cells resolve to; batch 50 added the MEASURED — CAPACITY UNMAPPED verdict (own-key demand against no mapped HC — IT product-model seats, the ASM alias gap) and the ZERO-DURATION / UNPARSEABLE-FREQ classification of the residue; batch 51 re-pointed the five dispositioned cells and repaired the engine's per-occurrence/period-bridge grammar — per-occurrence subset rates zeroed as symbolic and period-qualified totals bridged ×52/×12/×4/×1 — with the table re-derived to OVERLOAD 0); verifies the weak-anchor watchlist against virtual-gemba motion --full per-role annual demand; writes 01-model-company/workflows/weak-anchor-demand-verification.md (CONFIRMED / OVERLOAD / UNDER-UTILIZED / EXERCISED-THROUGH / MEASURED — CAPACITY UNMAPPED / ZERO-DURATION / UNPARSEABLE-FREQ / NO PARSED CADENCE verdicts; regenerate after any anchoring change)
    ├── verify-role-demand.py  Universal role-demand register generator (batch 52c, 2026-09-26, by direction "I want consistency") — generalizes the batch-49–51 watchlist instrument to ALL 259 chartered roles per universal-role-demand-verification-spec.md: engine-parity parts attribution in-process (no subprocess; one corpus pass) with the alias-surface fallback (ROLE_ALIASES + W36/W38/W39 + IT_SEATS reverse map, blocklist-guarded; since 52c priced only at the corpus's reconciliation grain — whole-cell or comma/semicolon-segment equality, never substring — dissolving the 52b run's three phantom OVERLOADs and restoring the EXERCISED-THROUGH verdict with the via-group named), attribution confidence (own-surface / alias-claimed / participant-only), state column (active / deferred-prepared OMO-TPS / dormant Trade design-load), peak-aware flagging, department roll-up, IT-seat HC mirror (IT_SEAT_HC from ITOM §9.1/§5.3 — 26 seats priced, vocabulary-umbrella seats deliberately UNPRICED as the ITOM seat-charter worklist), days-bridge armed for effort-worded cells only (inert: all 2,869 corpus day/week cells are elapsed windows); writes 01-model-company/workflows/role-demand-verification.md (regenerate after any anchoring/PA/ITOM change; --check byte-verifies; unpinned until Phase 3)
    ├── triage-role-demand.py  P2 disposition triage (batch 54, 2026-09-26) — source-tracked attribution (per-cell evidence behind every role's demand) triaging the register's active OVERLOAD rows into SCALE-ARTIFACT (field-exec × HQ charter) / CELL-AUDIT (concentration) / FIELD-SIGNAL (chain vs roster) / DISTRIBUTED (real-signal candidate) classes with batch-51-precedent disposition paths, the engine grammar-gap sensitivity markers (PER-UNIT? / PERIOD? — 19 of 79 rows), and the charter naming worklist ranked by unnamed hours (since batch 54 joined at the register's demand basis — every active-OVERLOAD row carries a queue row and the naming queue's sub-5% population IS the register's PARTIAL — UNNAMED class one-for-one); writes 01-model-company/workflows/role-demand-disposition-queue.md (--check byte-verifies; unpinned)
    ├── universal-role-demand-verification-spec.md  Universal role-demand verification spec (batch 52/52b/52c, 2026-09-26) — the doctrine (anchor / verified demand / named residual — one evidence standard for every seat regardless of work shape), the design (census join, engine-parity attribution, IT-seat pricing, residual taxonomy), phases P0–P3 with the pin protocol, and the batch-52b/52c implementation records (run-1 merge-artifact correction; run-2 substring-claim correction — the grain guard, the EXERCISED-THROUGH restore, the batch-54 queue join; elapsed-vs-effort bridge gating, IT_SEAT_HC)
    ├── fix-ta-analyst-reanchor.py  T&A Analyst re-anchor (batch 42, 2026-09-23) — reverted the wave-1 store-scaled elevation artifact (bogus 10,139% OVERLOAD) and re-anchored the HQ data-quality seat on the W5532 downstream-integration-quality step; weak 58 → 57
    ├── fix-owner-grain-promotions-4.py  Owner-grain promotion sweep III (batch 36, 2026-09-23) — Dark Store Operations Manager → DC Operations Manager (13), Marketing Manager → Marketing Operations Manager (15), Accounting Manager → Manager, GL & Consolidation (Assistant Controller) (12), IT Infrastructure Manager → IT Operations (FS/INFRA) (13); Class B 1,412 → 1,359, Class C 3,871 → 3,927, Class A 192-cluster tail → 147
...
| Legal Entities | 5 |
| Total Headcount | 6,911 active (200 stores × 29 + 600 DC + 511 HQ; the TO's 6,932 design retains the disabled—prepared Trade department and the deferred—prepared TPS and OMO build squads) |
...
| Workflows (total) | **5,433** across 188 value streams, 569 process areas | `01-model-company/workflows/value-stream-index.md` |
...
```
   ...
   │                                                   │
   │  erp-requirements.md ←→ workflows/ (5,433 WF)    │
   │       ↕                    ↕                      │
