...
│   ├── role-demand-verification.md  GENERATED universal role-demand register (batch 52b, 2026-09-26) — ALL 259 chartered roles measured against chartered capacity per universal-role-demand-verification-spec.md (verdict + residual + state + confidence columns, department roll-up; engine-parity parts attribution with alias-surface fallback; regenerate via 07-methodology/verify-role-demand.py — unpinned until Phase 3, do not hand-edit)
│   ├── role-demand-disposition-queue.md  GENERATED P2 disposition queue (batch 53, 2026-09-26) — the 79 active-OVERLOAD rows with per-cell evidence, auto-classified SCALE-ARTIFACT / CELL-AUDIT / FIELD-SIGNAL / DISTRIBUTED with the engine grammar-gap sensitivity marked (PER-UNIT? / PERIOD?), plus the charter naming worklist ranked by unnamed hours (regenerate via 07-methodology/triage-role-demand.py — do not hand-edit)
...
└── 07-methodology/              ← Technical system architecture and guidelines
    ...
    ├── ai-first-operating-guide.md   The AI-first operating guide — ERP-agnostic doctrine (ten laws, six layers, capability contracts), sourcing neutrality, autonomy ladder, agent factory, the EKB as persistent memory + consistency engine, the no-confusion decision system, and the seven-phase adoption path (v1.26)
    ├── validate-repo.sh              Cross-reference validation (85 checks)
    ...
    ├── verify-role-demand.py  Universal role-demand register generator (batch 52b, 2026-09-26, by direction "I want consistency") — generalizes the batch-49–51 watchlist instrument to ALL 259 chartered roles per universal-role-demand-verification-spec.md: engine-parity parts attribution in-process (no subprocess; one corpus pass) with the alias-surface fallback (ROLE_ALIASES + W36/W38/W39 + IT_SEATS reverse map, blocklist-guarded), attribution confidence (own-surface / alias-claimed / participant-only), state column (active / deferred-prepared OMO-TPS / dormant Trade design-load), peak-aware flagging, department roll-up, IT-seat HC mirror (IT_SEAT_HC from ITOM §9.1/§5.3 — 26 seats priced, vocabulary-umbrella seats deliberately UNPRICED as the ITOM seat-charter worklist), days-bridge armed for effort-worded cells only (inert: all 2,869 corpus day/week cells are elapsed windows); writes 01-model-company/workflows/role-demand-verification.md (regenerate after any anchoring/PA/ITOM change; --check byte-verifies; unpinned until Phase 3)
    ├── triage-role-demand.py  P2 disposition triage (batch 53, 2026-09-26) — source-tracked attribution (per-cell evidence behind every role's demand) triaging the register's active OVERLOAD rows into SCALE-ARTIFACT (field-exec × HQ charter) / CELL-AUDIT (concentration) / FIELD-SIGNAL (chain vs roster) / DISTRIBUTED (real-signal candidate) classes with batch-51-precedent disposition paths, the engine grammar-gap sensitivity markers (PER-UNIT? / PERIOD? — 19 of 79 rows), and the charter naming worklist ranked by unnamed hours; writes 01-model-company/workflows/role-demand-disposition-queue.md (--check byte-verifies; unpinned)
    ├── universal-role-demand-verification-spec.md  Universal role-demand verification spec (batch 52/52b, 2026-09-26) — the doctrine (anchor / verified demand / named residual — one evidence standard for every seat regardless of work shape), the design (census join, engine-parity attribution, IT-seat pricing, residual taxonomy), phases P0–P3 with the pin protocol, and the batch-52b implementation record (run-1 merge-artifact correction, elapsed-vs-effort bridge gating, IT_SEAT_HC)
