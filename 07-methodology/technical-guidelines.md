...
| HQ | 1 | ≥ 200 Mbps | ~511 active HQ staff (≈460 concurrent users at the ~90% ratio; the TO's 532-role design retains the disabled—prepared Trade department and the deferred—prepared TPS and OMO build squads), reporting, batch processing |
...
*Document Version: 3.8 | Date: 2026-09-23 | **OMO build squad deferred — prepared (2026-09-23 (ah)): the §2.2 HQ sizing row and the §3 employee-population row.** With BOPIS the only enabled online fulfillment option (registry CAP-F01) every sale completes as a regular POS sale and order routing is deterministic; the OMO build squad's 7 seats defer (IT 115 → 108 active; OM v3.28) and the active HQ population drops 518 → 511; per the cell's own ~90%-ratio precedent the concurrent estimate re-derives ≈466 → **≈460**, the ≥ 200 Mbps HQ floor and the ~640 Mbps WAN aggregate unaffected (sized with headroom at the 511 era); §3's employee row re-bases 6,918 → **6,911** with the design annotation (the TO's 532-role design retains the disabled—prepared Trade department and the deferred—prepared TPS and OMO build squads). No §1/§4–§6 changes. Prior 3.7 | Date: 2026-09-23 | **TPS build squad deferred — prepared (2026-09-23 (ad)): the §2.2 HQ sizing row and the §3 employee-population row.** With the trade desk disabled (x), the TPS build squad's 7 seats defer (IT 122 → 115 active; OM v3.25) and the active HQ population drops 525 → 518; per the cell's own ~90%-ratio precedent the concurrent estimate re-derives ≈473 → **≈466**, the ≥ 200 Mbps HQ floor and the ~640 Mbps WAN aggregate unaffected (sized with headroom at the 511 era); §3's employee row re-bases 6,925 → **6,918** with the design annotation (the TO's 532-role design retains the disabled—prepared Trade department and the deferred—prepared TPS build squad). No §1/§4–§6 changes. Prior 3.6 | Date: 2026-09-23 | **Seventy-ninth-wave consistency review (the §2.2 HQ sizing row).** The (x) trade-desk disablement re-based §3's employee-population row (6,932 → 6,925 active) but left §2.2's HQ bandwidth-sizing cell at the retired active canon ('~532 HQ staff (≈480 concurrent users)') — the cell whose own v3.4/v2.5 re-base precedent derives the concurrent estimate at the ~90% ratio on each headcount move; re-based to **~525 active HQ staff (≈473 concurrent users)** with the design annotation (the TO's 532-role design retains the disabled—prepared Trade department); the ≥ 200 Mbps HQ floor and the ~640 Mbps WAN aggregate are unaffected (sized with headroom at the 511 era). No §1/§3–§6 changes. Prior 3.5 | Date: 2026-09-23 | Trade-desk disablement (2026-09-23 (x)): the §3 employee-population row re-based 6,932 → 6,925 active (Trade / Account Management disabled — prepared, registry CAP-B01). Prior 3.4 | Date: 2026-09-14 | Structure-promotion re-base (profile v3.0 / TO v2.3): §2.1 headcount 6,762 → 6,911; §2.2 HQ row ~362 HQ staff (≈325 concurrent users) → **~511 HQ staff (≈460 concurrent users)** with the HQ link floor raised ≥ 100 → **≥ 200 Mbps** and the WAN aggregate ~540 → **~640 Mbps** (200 × 2 + 4 × 10 + 200; the same ~90% concurrency ratio). No §1/§3–§6 changes. Prior v3.3 (2026-09-14): §5 re-scoped to the two-tier sourcing doctrine (sourcing model v3.0): in-suite Oracle WMS/MSCA + Shipping/OTE (BoB WMS/TMS superseded), vendor-commodity release intake, statutory boundary re-worded for the built Payroll PH (human-confirmed filing acts; EBS ledger of record via postings), DR posture wording. No §1–§4/§6 changes. Prior v3.2 (2026-09-03): §6 blockquote continuation lines re-prefixed (the v3.1 note's second and third lines had lost their `>` markers — formatting only, no content change). Prior v3.1 (2026-09-03): §6 agentic runtime reference added with the agentic-AI extension (OM v2.1): tools-are-contracts, non-human identity & SoD, Tier-based autonomy ladder, statutory-boundary extension (human sign-off on filings), OT/POS exclusion, VS-128 registration & kill-switch, evaluation gates, audit-as-control-evidence, privacy/DPIA rules, foundation-model vendors as tier-1 TPRM, cost telemetry. Prior v3.0 (2026-09-03): §5 multi-vendor sourcing-architecture reference added with the hybrid capability-sourcing decision (single integration path, canonical contracts, inventory-ledger-of-record rule, POS-offline preservation, SEP paved road, vendor release intake, statutory boundary, DR posture). Prior v2.5 (2026-06-26): §2.2 HQ bandwidth row reconciled with the 2026-06-25 headcount change — `~357 HQ staff (≈320 concurrent users)` → `~362 HQ staff (≈325 concurrent users)` (the +5 Supply Chain & Logistics S&OP/IBP sub-team; concurrent estimate held at the same ~90% ratio). §2.1 already carried 6,762 employees. Prior v2.4: §3.1 fixed broken in-page anchor `#143-integration-touchpoints` → `#143-active-integration-touchpoints` to match the `### 14.3 Active Integration Touchpoints` heading in `model-company-profile.md` (same fix applied to NFR-012 in `erp-requirements.md`). Prior v2.3: §2.3 Data-retention row and §2.3 backup archive updated to **10 years (BIR per TRAIN/NIRC — Sec. 235, as amended by RA 10963)**; was 7 years. Prior v2.2: Integration diagram canonical reference updated; bank list reconciled to 4 banks (BDO, BPI, Metrobank, Chinabank); POS offline-capacity figure tied to the documented 2.0× peak factor (~933/store peak-day); counts reconciled with README.md*
