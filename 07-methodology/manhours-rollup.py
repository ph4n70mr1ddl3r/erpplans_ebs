...
DORMANCY_REGISTER = os.path.join(
    os.path.dirname(os.path.abspath(__file__)),
    "dormant-capability-register.json")


def load_dormancy():
    """The capability registry's machine twin (registry rule 4), plus the
    capability-sourcing register's squad-deferred estates: DISABLED — rows and
    'Build — squad deferred — prepared' §4 amendments -> dormant W/PA/VS sets,
    consumed to split live vs designed-capacity effort.

    Dormant capabilities keep their prepared designs in the corpus (disable ≠
    delete, registry rule 2), but their effort must not price the live
    organization. The register is curated — a registry Vehicles cell may cite
    support flows or mixed live/dormant estates that must NOT be marked (see
    the register's exclusions) — and is updated in the same change as any
    registry row flip (W5580) or sourcing-register §4 posture flip (W5515's
    register; the sourcing_register_deferrals section — the (bt) pass closed
    the seam where VS-60/VS-74/VS-77 carried the guarded 'held at designed
    capacity' banners while this tool priced them live).
    """
    reg = json.load(open(DORMANCY_REGISTER, encoding="utf-8"))
    dwf, dpa, dvs, state = {}, {}, {}, {}
    for cap in reg.get("capabilities", []):
        cid = cap["id"]
        state[cid] = cap.get("state", "DISABLED")
        for w in cap.get("dormant_workflows", []):
            dwf.setdefault(w.upper(), []).append(cid)
        for p in cap.get("dormant_process_areas", []):
            dpa.setdefault(p, []).append(cid)
        for v in cap.get("dormant_value_streams", []):
            dvs.setdefault(v, []).append(cid)
    for dfr in reg.get("sourcing_register_deferrals", {}).get("deferrals", []):
        did = dfr["id"]
        state[did] = dfr.get("state", "squad deferred — prepared")
        for v in dfr.get("dormant_value_streams", []):
            dvs.setdefault(v, []).append(did)
    return dwf, dpa, dvs, state
...
def main():
    ...
    dorm_wf, dorm_pa, dorm_vs, dorm_state = load_dormancy()
    class_counts = collections.Counter()
    workflows = []
    role_month = collections.defaultdict(lambda: [0.0, 0.0, 0, 0])  # low, high, wf_steps, auto
    role_month_live = collections.defaultdict(lambda: [0.0, 0.0, 0, 0])
    for rel, wid, wname, w in walk():
        mvs = re.match(r"^(VS-\d+)", rel.split("/")[2])
        mpa = re.match(r"^(PA-\d+\.\d+)", os.path.basename(rel))
        dcaps = sorted(set(
            dorm_wf.get(wid.upper(), [])
            + (dorm_pa.get(mpa.group(1), []) if mpa else [])
            + (dorm_vs.get(mvs.group(1), []) if mvs else [])))
        rec = {"file": rel, "id": wid, "name": wname, "header": w["header"],
               "frequency": w["frequency"], "steps": [], "irregular_rows": w["irregular_rows"],
               "dormant": bool(dcaps), "dormant_capabilities": dcaps}
    ...
    if args.workflow:
        ...
        if w["dormant"]:
            print(f"DORMANT — designed capacity (registry: {', '.join(w['dormant_capabilities'])}); "
                  f"excluded from live-capability totals")
        for s in w["steps"]:
    ...
    P("")
    P("Capability dormancy: workflows under registry DISABLED — rows and the "
      "capability-sourcing register §4's squad-deferred estates (input of record "
      "`07-methodology/dormant-capability-register.json`, capabilities + "
      "sourcing_register_deferrals) are split out — the live-capability totals "
      "price the organization; dormant effort is held as designed capacity "
      "(disable ≠ delete, registry rule 2).")
    P("")
    ...
    P("")
    # corpus totals — live capability estate vs dormant designed capacity
    live = [w for w in workflows if not w["dormant"]]
    dorm = [w for w in workflows if w["dormant"]]
    tot = [sum(w["monthly_hours"][0] for w in workflows),
           sum(w["monthly_hours"][1] for w in workflows)]
    ltot = [sum(w["monthly_hours"][0] for w in live),
            sum(w["monthly_hours"][1] for w in live)]
    dtot = [sum(w["monthly_hours"][0] for w in dorm),
            sum(w["monthly_hours"][1] for w in dorm)]
    def _fte(x):
        return x / BUSINESS_DAYS_PER_MONTH / HOURS_PER_WORKDAY
    P("## Corpus totals (cadence-denominated effort only)")
    P("")
    P(f"- **Live-capability effort (registry ENABLED set; dormant rows and "
      f"squad-deferred estates excluded): "
      f"{ltot[0]:,.0f}–{ltot[1]:,.0f} hours/month** "
      f"(≈ {_fte(ltot[0]):,.0f}–{_fte(ltot[1]):,.0f} FTE "
      f"at 173.6 h/FTE-month) across {len(live)} live workflows — "
      f"the headcount-basis figure")
    P(f"- Dormant designed capacity (registry DISABLED — rows + sourcing-register "
      f"deferrals, excluded from live): "
      f"{dtot[0]:,.0f}–{dtot[1]:,.0f} hours/month "
      f"(≈ {_fte(dtot[0]):,.1f}–{_fte(dtot[1]):,.1f} FTE) across {len(dorm)} dormant workflows")
    P(f"- All-corpus mechanical total (live + dormant): {tot[0]:,.0f}–{tot[1]:,.0f} hours/month")
    ...
    P("")
    P("## Dormant capability adjustments (registry DISABLED — rows + sourcing-register deferrals)")
    P("")
    P("Dormant capabilities keep their prepared designs in the corpus (disable ≠ delete, "
      "registry rule 2); their effort is held as designed capacity, not live load. "
      "Input of record: `dormant-capability-register.json` — updated in the same change "
      "as any registry row flip (W5580) or sourcing-register §4 posture flip (the "
      "sourcing_register_deferrals section: TPS/OMO, the (ad)/(ah) squad deferrals). "
      "Workflows mapped to several capabilities count "
      "once in the totals above and in each capability's row below.")
    P("")
    P("| capability | state | dormant workflows | dormant h/month | dormant FTE (high) |")
    P("|---|---|---:|---:|---:|")
    for cid in sorted({c for w in dorm for c in w["dormant_capabilities"]}):
        mine = [w for w in dorm if cid in w["dormant_capabilities"]]
        mlo = sum(w["monthly_hours"][0] for w in mine)
        mhi = sum(w["monthly_hours"][1] for w in mine)
        P(f"| {cid} | {dorm_state[cid]} | {len(mine)} | {mlo:,.0f}–{mhi:,.0f} | {_fte(mhi):,.1f} |")
    P("")
    P("## Top roles by derived monthly hours (live capability estate)")
    P("")
    P("| role | h/month (derived) | effort steps | automated steps |")
    P("|---|---:|---:|---:|")
    for role, (lo, hi, nst, nau) in sorted(role_month_live.items(), key=lambda kv: -kv[1][1])[:30]:
        P(f"| {role} | {lo:,.0f}–{hi:,.0f} | {nst} | {nau} |")
    P("")
    P("## Top workflows by derived monthly hours (live capability estate)")
    P("")
    P("| workflow | h/month | steps (effort/auto/cad-only/qual) |")
    P("|---|---:|---|")
    for w in sorted(live, key=lambda w: -w["monthly_hours"][1])[:30]:
    ...
    def csv_str():
        ...
        wr.writerow(["file", "workflow", "role", "monthly_hours_low",
                     ...
                     "live_or_dormant", "dormant_capabilities"])
        ...
        dorm_key = {(w["file"], w["id"]): w["dormant_capabilities"] for w in workflows}
        for (f, wid, role), c in sorted(by_wr.items()):
            dc = dorm_key.get((f, wid), [])
            wr.writerow([f, wid, role, f"{c[0]:.2f}", f"{c[1]:.2f}", c[2], c[3],
                         c[4], c[5], f"{c[6]:.1f}", f"{c[7]:.1f}",
                         "; ".join(c[8]),
                         "dormant" if dc else "live", ";".join(dc)])
        return buf.getvalue()
    ...
    def ranking_str():
        ...
        for i, w in enumerate(recs, 1):
            ...
            wr.writerow([i, w["id"], w["name"], area, w["file"],
                         ...
                         "dormant" if w["dormant"] else "live",
                         ";".join(w["dormant_capabilities"])])
        return buf.getvalue()
