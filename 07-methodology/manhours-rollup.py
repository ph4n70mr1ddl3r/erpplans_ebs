...
def load_dormancy():
    """The capability registry's machine twin (registry rule 4): DISABLED — rows
    -> dormant W/PA/VS sets, consumed to split live vs designed-capacity effort.
...
def main():
    ...
    P("")
    P("Capability dormancy: workflows under registry DISABLED — rows (input of record "
      "`07-methodology/dormant-capability-register.json`) are split out — the "
      "live-capability totals price the organization; dormant effort is held as "
      "designed capacity (disable ≠ delete, registry rule 2).")
    P("")
    ...
    P("")
    P(f"- **Live-capability effort (registry ENABLED set; dormant rows excluded): "
    ...
    P(f"- Dormant designed capacity (registry DISABLED — rows, excluded from live): "
    ...
    P("")
    P("## Dormant capability adjustments (registry DISABLED — rows)")
    P("")
    P("Dormant capabilities keep their prepared designs in the corpus (disable ≠ delete, "
      ...
      "as any registry row flip (W5580). Workflows mapped to several capabilities count "
