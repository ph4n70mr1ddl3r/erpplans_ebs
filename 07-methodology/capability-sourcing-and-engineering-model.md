...
> Companion to [`it-product-operating-model.md`](it-product-operating-model.md) (v3.11): how
...
| **Installation & home-service dispatch** | VS-12 | **Build** (2026-09-14; supersedes the BoB FSM row) | In-house dispatch build — remit (extend TPS vs new squad) at SIB/OM | CCP | None scheduled |
...
| Omnichannel order routing, split-order & mixed-basket fulfillment | VS-60 | **Build** (unchanged 2026-09-03) | In-house Order Orchestration product (OMO) | OMO | None — differentiating |
| Trade & project services coordination (job-site delivery, material staging/phased delivery, bulky install/haul-away) | VS-74, VS-77, VS-143 | **Build** (unchanged) | In-house Trade & Project Services platform (TPS) | TPS | None — differentiating |
...
| Foundation-model access (LLM APIs) | Cross-cutting (all agent use cases) | **Buy — commodity procurement** (not capability sourcing; the doctrine's sole vendor product line) | Vendor foundation models consumed through IAP-governed API edges | AAP with SEC (TPRM) | Philippine AI-regulation / NPC guidance change |
...
| **Configure-and-integrate** | WLI, SSP, CCP | Configure core **plus** product/vendor management re-pointed by the two-tier doctrine: the Oracle relationship and in-suite currency (RUP/CPU cadence), integration ownership for the **already-built platforms** (POS estate → SSP; ecommerce/loyalty/gift-card → CCP) and the in-house builds in their domain (store workforce → SSP; dispatch → CCP) |
| **Build** (squads) | OMO, TPS — plus the doctrine's new build scope (Payroll PH, store workforce, dispatch), squad/remit assignment at SIB/OM | Product Manager, Tech Lead, 3–4 software engineers, QA automation engineer; shared UX/product designer (SEP pool); matrixed IAP engineer and DP data analyst |
...
- **Vendor commodities** (LLM APIs, managed SOC, outsourced L1 contact center, hardware and
  license vendors): each has a **named product/vendor manager** in the owning team —
  contract and SLA owner, vendor roadmap intelligence, release-intake owner, escalation
...
| **Tier 2** (3,296) | **Bounded autonomy** | Agent acts inside hard guardrails (limits, whitelists, value caps); sampled human audit; auto-escalation on anomaly |
...
|---|---|
| [`it-product-operating-model.md`](it-product-operating-model.md) | The operating model this sourcing strategy reshapes (v3.11: 17 teams incl. AAP, archetypes per the two-tier doctrine, SEP, SIB, 122 FTE) |
