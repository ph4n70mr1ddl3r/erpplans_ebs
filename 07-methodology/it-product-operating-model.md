...
> [`capability-sourcing-and-engineering-model.md`](capability-sourcing-and-engineering-model.md) (v3.21).
...
> Since **v3.28 the two in-house build squads are deferred — prepared** (TPS 2026-09-23 (ad);
> OMO 2026-09-23 (ah), by direction): with BOPIS the only enabled online fulfillment option
> (channel-capability-registry CAP-F01) every sale completes as a regular POS sale and order
> routing is deterministic — the 122-FTE / 17-team design of record is retained per
> disable-not-delete, **108 FTE active**.
...
(188 value streams, 569 process areas, 5,433 workflows).
...
org's structure promotion (profile v3.0 §3.3; TO v2.5 §5.2: "Actual = 122"). Implementation-phase structures (SI governance,
...
TPS, created by sourcing decisions rather than value-stream families — both build squads
deferred — prepared, TPS 2026-09-23 (ad) and OMO (ah), their teams' VS assignments held as
designed capacity), 7 platform teams, and
...
| 5 | Finance & Treasury Systems | FIN | Stream-aligned (configure) | CFO (Controller, Treasurer, Tax) | 30 | 810 |
| 6 | Corporate, Governance & Asset Systems | CORP | Stream-aligned (configure) | VP Legal & Compliance; Facilities & Real Estate; Quality; Internal Audit & Risk | 35 | 925 |
| 7 | People Systems | PEO | Stream-aligned (configure) | CHRO | 16 | 445 |
| 8 | Order Orchestration | OMO | Stream-aligned (build; **squad deferred — prepared 2026-09-23 (ah)** — with BOPIS the only enabled fulfillment option (registry CAP-F01) every sale completes as a regular POS sale at the customer-selected store and routing is deterministic; the multi-source orchestration design activates with a second fulfillment origin) | Digital Commerce with Supply Chain | 1 | 24 |
| 9 | Trade & Project Services Platform | TPS | Stream-aligned (build; **squad deferred — prepared 2026-09-23 (ad)**) | VP Supply Chain (Fleet & Logistics) with Trade/Account Mgmt | 3 | 72 |
| | **Domain subtotal** | | | | **171** | **4,937** |
...
| | CIO Office (EA, Portfolio Governance, FinOps, Vendor Portfolio) | CIO | Enabling | CIO; CEO; Finance | 3 | 85 |
| | **Platform + CIO subtotal** | | | | **17** | **496** |
| | **Total** | | | | **188** | **5,433** |
...
| FIN | 29 (§4.4) + 1 (VS-86) | 810 |
| CORP | 12 (§4.6) + 22 (§4.7) + 1 (VS-115) + 1 (VS-24, batch 23) | 925 |
| PEO | 16 (§4.5) | 445 |
...
| CIO Office | 3 (VS-30, VS-113, VS-135) | 85 |
| **Total** | **171 + 17 = 188** | **4,937 + 496 = 5,433** |
...
**build** squads (OMO, TPS — both deferred — prepared, (ad)/(ah) — plus the doctrine's new
build scope — Payroll PH, store workforce, dispatch). All three start from the same 6-role
core below; the
...
**Build-squad shape (OMO, TPS — both squads deferred — prepared).** The two in-house products
are designed for stable software squads on the SEP paved road (sourcing model §5–§7); **both
squads are deferred — prepared**: the TPS squad (2026-09-23 (ad)) with the trade desk disabled,
and the OMO squad (2026-09-23 (ah), by direction) because every sale completes as a regular POS
sale — BOPIS (registry CAP-F01) is the only enabled online fulfillment option, checkout pins the
pickup store, and the multi-source orchestration workload (routing across stores/DCs/drop-ship/
dark stores, split orders) is the disabled—prepared estate that activates only with a second
fulfillment origin (CAP-F04–F09) or a marketplace channel (CAP-C03/C04); in the live posture the
already-built ecommerce platform's capability-configuration service (registry rule 3) and the
in-suite order flows carry the deterministic BOPIS path with no orchestration product. The shape
below is both squads' prepared design for re-enablement; consumer bulky install/haul-away stays
covered by the in-suite Field Service dispatch core and the CCP dispatch experience layer:
...
| **Software Engineering Platform (SEP)** | Head of Engineering; 2 DevEx engineers; 1 AppSec engineer; 1 QA automation lead; 1 UX/product designer; 1 build SRE | The paved road for in-house products (OMO, TPS — both build squads deferred — prepared, (ad)/(ah); SEP stands ready for the doctrine's remaining build scope — Payroll PH, store workforce, dispatch): golden-path templates, CI/CD, feature flags and telemetry, SAST/DAST/dependency/SBOM security gates, the shared test and contract-testing harness, ring-deployment infrastructure, production-readiness reviews, and the engineering career track. Full definition in [`capability-sourcing-and-engineering-model.md`](capability-sourcing-and-engineering-model.md) §6. Treats build squads as customers; nothing ships off the paved road without an ARB-recorded exception. |
...
### 6.4 Build-squad delivery (both squads deferred — prepared)
...
- **Build (both squads deferred — prepared (ad)/(ah)):** continuous delivery on the SEP ring model (internal → canary
...
| OMO (frozen at the pre-deferral design — squad deferred — prepared (ah)) | Routing decision latency; split-order/mixed-basket success rate; order-event end-to-end latency; DORA four (deployment frequency, lead time, MTTR, change-failure rate) |
| TPS (deferred — prepared (ad); KPIs frozen at the pre-deferral design) | On-time job-site/phased delivery; staging schedule adherence; technician-app adoption; DORA four |
...
### 9.1 Steady-state design (122 IT FTE design; 108 active)
...
| CORP | 1 | — (¹) | 2 | — | — (¹) | 1 | — | — | 4 |
| PEO | 1 | 1 | 2 | — | 1 | 1 | — | — | 6 |
| OMO | 1 | 1 (TL) | — | 4 | — (²) | 1 | — | — | 7 (deferred — prepared (ah)) |
| TPS | 1 | 1 (TL) | — | 4 | — (²) | 1 | — | — | 7 (deferred — prepared (ad)) |
| **Domain subtotal** | 9 | 8 | 22 | 9 | 6 | 9 | 3 | — | **52 active** (66 design incl. the deferred OMO and TPS squads) |
...
> TPS are built by deferred — prepared build squads (§5.1); their data analysts are matrixed from DP and their
...
> platform team. **v3.25: the TPS build squad's 7 seats defer — prepared (2026-09-23 (ad)) with the trade desk disabled; v3.28: the OMO build squad's 7 seats defer — prepared (2026-09-23 (ah)) with every sale completing as a regular POS sale (BOPIS-only, registry CAP-F01) — domain 52 + platform/CIO 56 = 108 active; the 122-design is retained** The v3.12 mix re-point (§5.1/§5.4) converts WLI's second product/vendor
...
| Industry benchmark | ~104–173 (1.5–2.5% of the 6,911 active headcount); lean outsourced floor ~60–70 | `headcount-reality-check.md` §3.2 |
| This design, v1.x (unified model) | 80 | OM v1.x §9 |
| This design, v2.0 (hybrid) | 115 | OM v2.0 §9 |
| This design, v2.1 (hybrid + agentic) | 122 | §9.1 |
| This design, v3.25 (TPS build squad deferred — prepared) | 115 active | §9.1 |
| This design, v3.28 (OMO build squad deferred — prepared) | 108 active | §9.1 |
...
### 9.3 Phased build-up (50 → 122 design; 108 active)
...
| **Phase 1 — Field, backbone & first build** | FS regional field technicians and L2 (the archipelago gap); INFRA to full strength; IAP build-out toward 9; SEC security engineering + TPRM analyst; DP data-contract steward; SEP completed (AppSec, QA-automation lead, designer, build-SRE); **OMO squad #1** — deferred — prepared (ah): stands up only when a second online fulfillment origin (CAP-F04–F09) or a marketplace channel (CAP-C03/C04) enables — until then routing is deterministic BOPIS on the already-built platform + in-suite order flows | +35 | ~90 |
| **Phase 2 — In-suite adoption, second build, agentic platform & depth** | Oracle WMS/MSCA + Shipping/OTE adoption (WLI analyst depth); Payroll PH and store-workforce builds (SSP with PEO) and the dispatch build (CCP) — per the two-tier doctrine; **TPS squad #2** (deferred — prepared 2026-09-23 (ad); stands up at CAP-B01 re-enablement); **AAP stand-up** (lead, agent engineers, evaluation engineer — the crawl-phase read-only agents); remaining domain functional-analyst depth (MSC/SSP/FIN); IAP final engineer; FinOps analyst | +32 | 122 |
...
| [`workflows/value-stream-index.md`](../01-model-company/workflows/value-stream-index.md) | The 188-VS / 569-PA / 5,433-WF catalog this model assigns to products |
...
| [`capability-sourcing-and-engineering-model.md`](capability-sourcing-and-engineering-model.md) | The sourcing decision gate, Capability Sourcing Register, build-squad engineering standard, and SEP definition this v3.31 model is built on |
...
12-team design at 80 FTE. Downstream: `optimal-table-of-organization.md` v3.5 (HQ 511 active /
