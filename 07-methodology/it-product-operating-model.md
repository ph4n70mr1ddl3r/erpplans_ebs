...
> [`capability-sourcing-and-engineering-model.md`](capability-sourcing-and-engineering-model.md) (v3.0).
...
`model-company-profile.md` §14.1; supersedes the 2026-09-03 hybrid posture). Implementation-phase structures (SI governance,
...
> **Configure-and-integrate teams (WLI, SSP, CCP) add:** **product/vendor management** (1–2
> per team) re-pointed by the two-tier doctrine — WLI: the Oracle supply-chain relationship
> and in-suite currency for Oracle WMS/MSCA + Shipping/OTE; SSP: the already-built POS
> platform and the store-workforce build; CCP: the already-built ecommerce/loyalty platforms
> and the dispatch build — owning contracts/SLAs where vendors remain (LLM APIs,
> commodities), roadmap intelligence, release intake, upgrade currency, and the TPRM
> (VS-161) liaison; plus added functional-analyst depth for the in-suite add-ons'
...
| **Product/Vendor Manager** | Seats unchanged in the configure-and-integrate archetype (WLI, SSP, CCP). Under the two-tier doctrine the remit is the Oracle relationship + in-suite currency (WLI), the already-built POS platform + workforce build (SSP), and the ecommerce/loyalty platforms + dispatch build (CCP), plus the commodity vendor line (LLM APIs, managed services). Contract/SLA owner, roadmap intelligence, release-intake owner, TPRM liaison, currency KPI owner (sourcing model §8). |
...
- **Vendor commodities (LLM APIs, managed services):** vendor changes are intake-checked by
  the team's product/vendor manager against contract and TPRM terms. No capability products
  are bought under the two-tier doctrine, so no BoB release train exists; the already-built
  platforms (POS, ecommerce, loyalty) release on the build cadence below, owned by their
  teams.
...
| [`capability-sourcing-and-engineering-model.md`](capability-sourcing-and-engineering-model.md) | The sourcing decision gate, Capability Sourcing Register, build-squad engineering standard, and SEP definition this v3.11 model is built on |
...
12-team design at 80 FTE. Downstream: `optimal-table-of-organization.md` v2.1 (HQ 511 /
total 6,911), `model-company-profile.md` v2.28, `technical-guidelines.md` v3.3,
`capability-sourcing-and-engineering-model.md` v3.0.*
