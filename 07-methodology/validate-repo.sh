...
#   * manhours-rollup.{md,json,csv} — generated by manhours-rollup.py (the
#     2026-09-25 (bg) direction), which had report/json/csv writers but NO
#     --check mode; the 2026-09-25 (bk) sixth policy-pass sweep (by direction:
#     review everything; implement recommendations) added the mode (in-memory
#     re-derivation of all three artifacts + byte-compare — the same harness
#     class as the two reports above) and wired it here, closing the last
#     generated-analysis surface whose freshness no rule read.
