...
#   (c) population assertions — the parse must see all 5,430 Pain-bearing workflows, so a
...
if len(pain_wfs) != 5430:
    bad += 1
    print(f"BAD|Pain-section parse population {len(pain_wfs)} != 5430 workflows \u2014 parser or corpus drift, arms void")
...
# PA files' 5,430 intra-file anchors were the only ones any rule scanned, while the 27
...
# the Quick-Stats table (188 / 569 / 5,430 / 1,396 / 3,299 / 758 / the 'Classified total'
...
for i, ln in enumerate(dep.split('\n'), 1):
    if 'remains unclassified' in ln or 'pending criticality review' in ln:
        errs.append(f"workflow-dependency-map.md:{i} asserts an unclassified/pending state, but all 5,430 workflows have been classified (2026-06-28 Full-Coverage Confirmation Pass; 2026-09-02 post-catalog confirmation of W5497–W5510; 2026-09-03 W5511; 2026-09-03 W5512–W5514; 2026-09-03 W5515–W5517; 2026-09-10 W5574; 2026-09-21 W5575–W5577)")
...
if [ "$C27_ERRS" -eq 0 ]; then
    ok "No unclassified-workflow claims in workflow-dependency-map.md and no retired corpus-total literal (any workflow-adjacent integer in the self-re-arming [5,320, canon) band) in any PA file's live prose (canon: 5,430 unique; Part C added by the 2026-09-09 fourteenth-wave review after PA-128.3's W5512 Volume row shipped '5,370-workflow inventory' through batches 15–23 on a surface no check read; band made corpus-derived by the 2026-09-10 seventeenth-wave review after the same row stranded at the then-canonical 5,426 with the static band blind to it)"
else
...
# semantically-mangled Owner cells rewritten in their PA files): ALL 5,430
...
case "$C71_CENSUS_OUT" in
  *"CENSUS workflows=5430 owner_resolved=5430 owner_uncharted=0 owner_uncharted_forms=0 uncharted_forms=0 ctl_owner_cells=808 ctl_owner_resolved=808 ctl_owner_uncharted=0 ctl_owner_uncharted_forms=0 prose_role_uncharted=0 prose_role_forms=0"*) C71_CENSUS_OK=1;;
esac
