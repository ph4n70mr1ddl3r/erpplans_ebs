...
#   D. (2026-09-16 forty-third-wave review) Automation Opportunity / Pain Points bullets
#      are paren-balanced per line — the same review found four Automation bullets quoting
#      their step text truncated mid-paren (the generator-era truncation class Check 43's
#      own history records for Controls lines, one section over) plus one corrupted
#      Pain-Points tail; part B's per-line style extended to the two sibling sections
...
for f in files:
    ...
    for bi, block in enumerate(text.split("\n\n")):
        if block.startswith("|") or block.lstrip().startswith(("#", "---")):
            continue  # tables are row-scoped, headings/HRs carry no prose parens
        block = re.sub(r"`[^`]*`", "", block)
        if block.count("(") != block.count(")"):
            first = block.strip().splitlines()[0][:80] if block.strip() else ""
            bad += 1
...
vehicle_literals = [
    (r'ERP/Audit Management System', 'retired generic audit vehicle (the H11 surface: adopted as ICM 2026-09-16, re-dispositioned to the in-house Audit & GRC platform build 2026-09-22 per Vision VF-2 — fit-gap H11 BUILD)'),
    ...
    # 2026-09-16 eighth-pass review: the remaining adopted-vehicle classes named and the learning-platform posture trued onto the in-suite OLA (fit-gap §4 resolution 32).
    (r'Inventory/ATP system', 'retired generic ATP vehicle (adopted: Oracle Global Order Promising for the availability sense — fit-gap D15; the stock-status/alert sense rides INV)'),
    (r'(?i)demand planning module', 'retired generic demand-planning vehicle, any case (adopted: Demantra on the ASCP planning stack — fit-gap C5; the forty-fifth-wave review trued the sentence-initial/title-case rows the case-sensitive literal let through)'),
    (r'(?i)supply planning module', 'retired generic supply-planning vehicle (adopted: the C5 ASCP planning stack — the replenishment-date source is the ASCP planning run)'),
    (r'Transportation Management System', 'retired BoB TMS vehicle (adopted: in-suite Shipping/Transportation Execution — fit-gap C14, the BoB TMS row superseded)'),
    (r'\(TMS\)', 'retired BoB TMS vehicle (adopted: in-suite Shipping/Transportation Execution — fit-gap C14, the BoB TMS row superseded)'),
    (r'\bTMS\b', 'retired bare TMS abbreviation (adopted: in-suite Shipping/Transportation Execution for execution/tender/rate/audit senses, the in-house Route Optimization Engine for route-optimization/dispatch senses — fit-gap C14; the forty-fifth-wave review trued the ~49 surviving tokens)'),
    (r'(?i)quality module(?! management)(?! \(VS-\d)', 'retired generic quality vehicle, bare form (adopted: Oracle Quality — fit-gap C12; the forty-fifth-wave review trued the Quality Module/W110 and <vendor|product|supplier> quality-module rows; the genre enumeration labels stand)'),
    (r'(?i)quality management module(?! \(VS-\d)', 'retired generic quality vehicle, compound form (adopted: Oracle Quality — fit-gap C12; the forty-fifth-wave review trued the PA-01.1 bolded row; the Quality Management Module (VS-31) genre label stands)'),
    (r'(?i)warranty module', 'retired generic warranty vehicle, bare form (adopted: Oracle Install Base + Service Contracts — fit-gap D16; the forty-fifth-wave review trued the PA-12.1 ST row and the PA-09.3 W33 row)'),
    (r'(?i)vendor rebate module', 'retired generic rebate vehicle, lowercase form (adopted: Oracle Trade Management — fit-gap B10; the forty-fifth-wave review trued the PA-15.1 accrual row; the (VS-39) genre enumeration labels stand)'),
    (r'(?i)(?<!yield/)revenue module', 'retired generic revenue vehicle (the A11 surface: second-pass adopted the in-suite revenue-schedule capability, naming trued 2026-09-22 to Receivables Revenue Management per Vision VF-1 — fit-gap A11; the forty-fifth-wave review trued the VS-157 ST rows and the PA-13.1/14.1/15.1 rows; the storage-PMS yield module stands as VS-174 domain naming)'),
    (r'(?i)ERP maintenance module', 'retired generic maintenance vehicle (adopted: Oracle eAM — fit-gap F5; the forty-fifth-wave review trued the PA-07.1/PA-08.3/PA-09.1 rows)'),
    (r'(?i)equipment maintenance module(?! \(EAM\))', 'retired generic maintenance vehicle (adopted: Oracle eAM — fit-gap F5; the forty-fifth-wave review trued the PA-09.1/PA-07.2 rows; the (EAM)-glossed row stands)'),
    (r'integrated with maintenance module', 'retired generic maintenance vehicle (adopted: Oracle eAM — fit-gap F5; the forty-fifth-wave review trued the PA-07.2 checklist-app row)'),
    (r'agreement/lease module', 'retired generic lease vehicle (adopted: Oracle Property Manager per the W117 canon — fit-gap F3; the forty-fifth-wave review trued the PA-07.1 concessionaire touchpoint; the storage-PMS shorthand stands as VS-174 domain naming)'),
    (r'calibration management system / CMMS', 'retired generic calibration vehicle (adopted: Oracle eAM as the CMMS-of-record — fit-gap F5; the forty-fifth-wave review trued the PA-115.1 deploy step and W3621 ST row)'),
    (r'Training LMS', 'retired bare-LMS enumeration (adopted: OLA — fit-gap E2; the forty-fifth-wave review trued the surviving bare forms)'),
    (r'training LMS', 'retired bare-LMS enumeration (adopted: OLA — fit-gap E2)'),
    (r'training/LMS', 'retired bare-LMS enumeration (adopted: OLA — fit-gap E2)'),
    (r'LMS certification', 'retired bare-LMS enumeration (adopted: OLA — fit-gap E2)'),
    (r'Seller portal; LMS', 'retired bare-LMS enumeration (adopted: OLA — fit-gap E2)'),
    (r'app; LMS;', 'retired bare-LMS enumeration (adopted: OLA — fit-gap E2)'),
    (r'\(Document/LMS\)', 'retired bare-LMS enumeration (adopted: OLA — fit-gap E2)'),
    (r'stack \(LMS,', 'retired bare-LMS enumeration (adopted: OLA — fit-gap E2)'),
    (r'WMS/LMS productivity', 'retired ambiguous LMS abbreviation (the labor-management sense: WMS Labor Management — fit-gap C6; the learning sense would be OLA — fit-gap E2)'),
    (r'; LMS; biometric', 'retired bare-LMS enumeration (adopted: OLA — fit-gap E2)'),
    (r'results in quality management system', 'retired generic quality vehicle (adopted: Oracle Quality — fit-gap C12; the supplier-side QMS-review instances stand)'),
    (r'ERP Quality (Management )?module', 'retired generic quality-vehicle name (adopted: Oracle Quality — fit-gap C12)'),
    (r'(?<!Oracle )Learning Management System', 'retired generic learning vehicle (adopted: Oracle Learning Management — fit-gap E2; the registered W5525 title, enumeration labels and external/vendor LMS instances stand)'),
    (r'per W74 \(learning and development\)', 'mis-pointed L&D cross-reference (W74 is the T&E expense workflow; the learning workflow is W51 — corrected by the eighth-pass review)'),
    (r'Maintenance work order (?:system|module)', 'retired generic maintenance-work-order vehicle (adopted: Oracle eAM — fit-gap F5)'),
    (r'Facilit(?:y|ies) maintenance work order (?:system|module)', 'retired generic facility-work-order vehicle (adopted: Oracle eAM — fit-gap F5)'),
    (r'Equipment maintenance request system', 'retired generic maintenance-request vehicle (adopted: Oracle eAM work requests — fit-gap F5)'),
    (r'Monthly PM scheduling system', 'retired generic PM-scheduling vehicle (adopted: Oracle eAM PM scheduling — fit-gap F5)'),
    (r'PM work order system with', 'retired generic PM-work-order vehicle (adopted: Oracle eAM — fit-gap F5)'),
    (r'Work Order module for renovation', 'retired generic project-work-order vehicle (renovation cost/CIP: Oracle Projects per the coverage-map Engineering & Construction row; task scheduling stays PMO tooling)'),
    (r'CAPA tracking integration', 'retired generic CAPA vehicle (adopted: Oracle Quality — fit-gap C12)'),
    (r'Vendor quality CAPA (?:module|system)', 'retired generic vendor-CAPA vehicle (adopted: Oracle Quality — fit-gap C12)'),
    (r'Supplier quality management system', 'retired generic supplier-quality vehicle (adopted: Oracle Quality — fit-gap C12)'),
    (r'CAPA module \(W110\)', 'retired generic CAPA vehicle (adopted: Oracle Quality — fit-gap C12)'),
    (r'Demand forecasting module', 'retired generic demand-forecasting vehicle (adopted: Demantra — fit-gap C5)'),
    (r'Demand planning / forecasting module', 'retired generic demand-planning vehicle (adopted: Demantra — fit-gap C5)'),
    (r'ERP Forecasting Module', 'retired generic forecasting vehicle (adopted: Demantra — fit-gap C5)'),
    (r'Demand Forecasting Engine', 'retired generic demand-forecasting vehicle (adopted: Demantra — fit-gap C5)'),
    (r'Cash [Ff]low [Ff]orecasting (?:tool|module)', 'retired generic cash-forecasting vehicle (adopted: Oracle Treasury XTR — fit-gap A8)'),
    (r'Treasury management system (?:with|for|—)', 'retired generic treasury vehicle (adopted: Oracle Treasury XTR + CE — fit-gap A8; the VS-105 domain-label row stands)'),
    (r'treasury intercompany module', 'retired generic IC vehicle (adopted: AGIS — fit-gap A7)'),
    (r'ERP Intercompany [Mm]odule', 'retired generic IC vehicle (adopted: AGIS — fit-gap A7)'),
    (r'Intercompany [Mm]odule \((?:W14|IC-001)', 'retired generic IC vehicle (adopted: AGIS — fit-gap A7; the VS-72 domain-label row stands)'),
    (r'Contract management module \(W402\)', 'retired generic contract vehicle (adopted: Oracle Procurement Contracts — fit-gap B12; the W1225/W1308 customer-side rows stand)'),
    (r'ERP Item Master Module', 'retired generic item-master vehicle (adopted: Oracle Product Hub — fit-gap C1)'),
    (r'ERP Vendor Master Module', 'retired generic vendor-master vehicle (adopted: Oracle SLM — fit-gap B6)'),
    (r'ERP UOM Master / Item Master Module', 'retired generic master vehicle (adopted: INV UOM + Oracle Product Hub — fit-gap C1/C2)'),
    (r'Item master module', 'retired generic item-master vehicle (adopted: Oracle Product Hub — fit-gap C1)'),
    (r'Lease Administration Module', 'retired generic lease-admin vehicle (adopted: Oracle Property Manager — fit-gap F3; ROU schedules per the A12 EXT)'),
    (r'Lease management system', 'retired generic lease vehicle (adopted: Oracle Property Manager — fit-gap F3)'),
    (r'[Ll]ease accounting module', 'retired generic ROU vehicle (adopted: the GL/FA ROU schedules — the A12 EXT)'),
    (r'sublease accounting module', 'retired generic ROU vehicle (adopted: the GL/FA ROU schedules — the A12 EXT)'),
    (r'Applicant tracking module', 'retired generic applicant-tracking vehicle (adopted: Oracle iRecruitment — fit-gap E2)'),
    (r'Applicant tracking system linked', 'retired generic applicant-tracking vehicle (adopted: Oracle iRecruitment — fit-gap E2)'),
    (r'Progressive markdown engine', 'retired generic markdown vehicle (adopted: Advanced Pricing — the QP markdown schedule)'),
    (r'Markdown module', 'retired generic markdown vehicle (adopted: Advanced Pricing — the QP markdown schedule)'),
    (r'Pricing/markdown module', 'retired generic pricing vehicle (adopted: Advanced Pricing)'),
    (r'Margin analysis module', 'retired generic margin vehicle (adopted: Oracle In-Memory Cost Management — fit-gap C15)'),
    (r'Fulfillment cost allocation and margin analysis module', 'retired generic margin vehicle (adopted: Oracle In-Memory Cost Management — fit-gap C15)'),
    (r'Deduplication engine \(W253\)', 'retired generic dedup vehicle (adopted: TCA DQM — fit-gap B6)'),
    (r'Customer Data Platform \(CDP\) deduplication engine', 'retired CDP-as-master-dedup naming (customer-master dedup: TCA DQM — fit-gap B6; engagement identities stay on the in-house CDP)'),
    (r'Deduplication Engine', 'retired generic dedup vehicle (adopted: Oracle SLM duplicate check — fit-gap B6)'),
    (r'Training management module linked', 'retired generic training vehicle (adopted: Oracle Learning Management — fit-gap E2)'),
    (r'Training management module, employee master', 'retired generic training vehicle (adopted: Oracle Learning Management — fit-gap E2)'),
    (r'training management system:', 'retired generic training vehicle (adopted: Oracle Learning Management — fit-gap E2)'),
    (r'ERP HR/Payroll module', 'retired ERP-payroll implication (payroll is the in-house Payroll PH build — fit-gap E5; Oracle Payroll unadopted)'),
    (r'ERP Payroll module', 'retired ERP-payroll implication (payroll is the in-house Payroll PH build — fit-gap E5; Oracle Payroll unadopted)'),
    (r'Route planning module', 'retired generic routing vehicle (adopted: the in-house Route Optimization Engine — fit-gap C14)'),
    (r'Route optimization module', 'retired generic routing vehicle (adopted: the in-house Route Optimization Engine — fit-gap C14)'),
    (r'route optimization (?:engine|software)', 'retired generic routing vehicle (adopted: the in-house Route Optimization Engine — fit-gap C14)'),
    (r'Load planning module', 'retired generic load-planning vehicle (adopted: in-suite Shipping/Transportation Execution — fit-gap C14)'),
    (r'Fleet management module, driver', 'retired generic fleet vehicle (vehicle register: FA; maintenance: Oracle eAM — fit-gap F5; telematics stay feeds)'),
    (r'Calendar/alert system for registration expiry', 'retired generic date-alert vehicle (adopted: Oracle Alert over the EIT register — fit-gap G9)'),
    (r'HRIS Succession Module', 'retired generic succession vehicle (adopted: Oracle Succession Planning — fit-gap E2)'),
    (r'ERP [Ll]oyalty module', 'retired ERP-loyalty implication (the loyalty engine is the already-built in-house stack — fit-gap D9; TCA is the member master)'),
    (r'ERP intercompany module', 'retired generic IC vehicle (adopted: AGIS — fit-gap A7)'),
    (r'[Rr]evenue recognition module', 'retired generic revenue vehicle (A11: Receivables Revenue Management in-suite AR per Vision VF-1)'),
    (r'Revenue Recognition Module', 'retired generic revenue vehicle (A11: Receivables Revenue Management in-suite AR per Vision VF-1)'),
    (r'ERP Subscription Billing module', 'retired generic subscription vehicle (A11: Receivables Revenue Management in-suite AR per Vision VF-1)'),
    (r'Rebate tracking module', 'retired generic rebate vehicle (adopted: Oracle Trade Management — fit-gap B10)'),
    (r'Monthly rebate accrual engine', 'retired generic rebate vehicle (adopted: Oracle Trade Management — fit-gap B10)'),
    (r'Quality inspection module', 'retired generic quality-inspection vehicle (adopted: Oracle Quality — fit-gap C12)'),
    (r'warranty tracking module', 'retired generic warranty vehicle (adopted: Oracle Install Base — fit-gap D16)'),
    (r'e-Procurement module for RFP', 'retired generic sourcing vehicle (adopted: Oracle Sourcing — fit-gap B5)'),
    (r'Compliance case management system', 'retired generic compliance-case vehicle (the H11 surface: the in-house Audit & GRC platform build per Vision VF-2)'),
    (r'Oracle Internal Controls Manager', 'retired vehicle — H11 re-dispositioned to the in-house Audit & GRC platform build 2026-09-22 per Vision VF-2 (AMW obsolete, not installed on either installation of record; fit-gap H11 BUILD)'),
    (r'\bICM\b', 'retired vehicle short form — see Vision VF-2 execution (fit-gap H11 BUILD)'),
    (r'Revenue Management and Invoicing', 'retired vehicle name — no such R12.2 application; the A11 capability is Receivables Revenue Management in-suite AR (Vision VF-1 execution, 2026-09-22)'),
    (r'RM&I', 'retired vehicle short form — see Vision VF-1 execution (Receivables Revenue Management, fit-gap A11)'),
    (r'Yard Management System \(YMS\)', 'retired generic yard vehicle (adopted: Oracle WMS Yard Management — fit-gap C6)'),
    (r'Route optimization system processing', 'retired generic routing vehicle (adopted: the in-house Route Optimization Engine — fit-gap C14)'),
    (r'route optimization engine calculates', 'retired generic routing vehicle (adopted: the in-house Route Optimization Engine — fit-gap C14)'),
    (r'software/CMMS', 'retired generic CMMS vehicle (adopted: Oracle eAM — fit-gap F5)'),
    (r'CMMS PM auto-generation', 'retired generic CMMS vehicle (adopted: Oracle eAM — fit-gap F5; the VS-138 domain-label rows stand)'),
    (r'BMS→CMMS', 'retired generic CMMS vehicle (adopted: Oracle eAM — fit-gap F5; the VS-138 domain-label rows stand)'),
    (r'OBD/fault-code → CMMS', 'retired generic CMMS vehicle (adopted: Oracle eAM — fit-gap F5; the VS-138 domain-label rows stand)'),
    (r'Lease & Finance Management rental contracts & reservations', 'retired L&FM product-mechanics vocabulary (122oklug documents lease contracts with rental-period billing — rent streams and evergreen rentals; fit-gap A13)'),
    (r'issued-but-unreleased', 'retired AP Bills Payable vocabulary (122apug: issued but not matured — the Future Dated Payments balance definition and the Maturity Date Report; fit-gap A3)'),
    (r'ERP trade account module', 'retired generic trade-incentive vehicle (adopted: Oracle Incentive Compensation — fit-gap H14, the coverage register §5 vehicle of record for trade-customer allowances; W1549 touchpoint + step-2a re-pointed by the seventy-fourth-wave review 2026-09-23)'),
    (r'~99,000 customers', 'retired population mislabel (the W330 canon counts PEOPLE on premises — ~467 customers + 29 staff per store ≈ ~99,000+; the customers-only figure is ~93,400; W5536/W5555/classification-register cells re-pointed by the seventy-fourth-wave review 2026-09-23)'),
]
...
if [ "${C46_BAD:-1}" -eq 0 ]; then
    ok "No stale tender-mix/fleet/SKU figures or superseded statutory citations (RA/ATC/DO literals) in current-state prose, and no retired generic-vehicle literals in the workflow corpus — every 'X module/system' row names the fit-gap register's adopted vehicle (vehicle-literal arm added by the 2026-09-16 fortieth-wave/seventh-pass review: the six audit passes adopted ICM/eAM/Property Manager/Procurement Contracts/Install Base/Trade Management/iExpenses/OAB/Performance Management/iRecruitment/CE/GOP but left ~60 workflow rows naming the superseded generic vehicles, incl. rows inside the adoptions' own cited workflows W808/W27/W89; extended by the eighth-pass review (2026-09-16) with the 'Inventory/ATP system' (GOP/INV), 'demand planning module' (Demantra), 'Transportation Management System (TMS)' (in-suite OTE), generic quality-system (Oracle Quality) and vehicle-sense 'LMS' (OLA — the vendor-SaaS posture retired against the resolution-5 'never bought' canon) retired forms plus the 'per W74 (learning and development)' mis-pointed L&D cross-reference; extended by the forty-fifth-wave review (2026-09-16) with the dodge-forms the per-form literals let through — the case-insensitive demand-planning literal and the supply-planning sibling (C5), the bare-TMS token (C14 OTE / the in-house Route Optimization Engine), the bare/compound quality forms (C12), bare warranty (D16), lowercase vendor-rebate (B10), the revenue-module rows (A11 RM&I), the ERP/equipment-maintenance and calibration-management forms (F5), the W117-pointed agreement/lease row (F3) and the bare-LMS enumerations (E2, incl. the WMS/LMS Labor-Management sense), each with its genre-label exemption named; scoped to workflows/** because the doctrine documents legitimately quote the retired forms in the sweeps' own cascade, and the '(VS-138)'-style domain-label cross-references stand per the sweep's adjudication; extended by the 2026-09-17 fifty-second-wave plain-wording sweep with the +68 retired forms whose different wordings dodged the exact-literal guards — the same superseded vehicles in other words (maintenance/facility/PM work-order and equipment-request rows → eAM F5, CAPA/supplier-quality and quality-inspection → C12, demand-forecasting → C5, cash-forecasting/treasury → A8 XTR/CE, intercompany → A7 AGIS, vendor/service-agreement contracts → B12, masters → C1/B6/C2, leases → F3 + the A12 EXT, applicant-tracking → E2, markdown → Advanced Pricing, margin → C15, dedup → B6 TCA DQM/SLM, training/succession → E2, ERP-payroll posture conflicts → the in-house Payroll PH build E5, ERP-loyalty conflicts → the already-built stack D9, revenue-recognition/subscription → A11, rebate → B10, CMMS residuals → F5, compliance-case → H11, warranty tracking → D16, construction-RFP → B5 Sourcing, yard → C6 WMS Yard, route planning → the in-house Route Optimization Engine, load planning → in-suite OTE C14, the W1148 fleet row → FA + eAM, registration-expiry alerts → G9 Oracle Alert; every added literal verified at zero pre-arm, the plain-word EBS module naming and domain-label cross-references left standing per the wave's adjudication; the 2026-09-21 sixty-second-wave manuals sweep added the two product-mechanics vocabulary literals — 'L&FM rental contracts & reservations' (122oklug documents lease contracts with rental-period billing: rent streams and evergreen rentals) and 'issued-but-unreleased' (122apug's phrase is 'issued but not matured'); and the 2026-09-23 seventy-fourth-wave review added the registry-admission pass's two retired literals — 'ERP trade account module' (the W1549 generic trade-incentive vehicle; adopted Oracle Incentive Compensation per fit-gap H14, the coverage register §5 vehicle of record for trade-customer allowances) and the '~99,000 customers' population mislabel (the W330 canon counts people on premises — customers + staff — the customers-only figure being ~93,400)')"
else
...
echo "    $C51_N"
if [ $C51_RC -eq 0 ]; then
    ok "No stale headcount literals, dept-team mismatches, Volume-product defects, or DC-catchment drift (guard mode of reconcile-staffing-claims.py, --guard armed by the 2026-09-09 sixteenth-wave review after two latent checker false positives were found printing under the unarmed run; 21 claim spots + 3 Volume rows repaired 2026-08-29; DC-catchment average/range claims re-derived from the profile's §3.1/§3.2 tables and the retired 20–60-range / ~40-average / ~40–50 forms retired by the sixteenth wave; the register re-based to the promoted 511-HQ §3.3 and the pre-promotion 362-HQ-subtotal family — the '362 HQ' forms, the old six-function enumeration, the 5-person trade register — retired across PA files + VS READMEs by the 2026-09-14 twenty-ninth-wave review, the stragglers the promotion's 6,762-total sweep left behind; the eighty-sixth wave trued the arm hit-messages to the post-(ah) canon (6,911/HQ 511/three-deferral — they still taught 6,918/HQ 518 as current), added 'active HQ 518' to the retired-HQ-active bans with the corrected PA-22.1 (x)-endpoint anchor required (the recital's 518 was the (ad) value mislabeled as (x)), and gained the retired derived-figure arm ('~13,836' banned on PA/README lines, the '~13,822 hours (annual reassessment)' anchor required at PA-30.3's annual-estimate cell — the half-repaired summary line the headcount arms cannot see); the eighty-seventh wave found the guard's own dead pair-check — the PA-34.1 laptop-refresh special case compared its anchor against the retired '518 active HQ + DC office staff' form while the (ah) re-point had moved the anchors-list entry itself to the 511 form, so the count-both-cells arm could never fire (proven by injection: stripping the touchpoint anchor left the guard silent) — the comparison re-pointed to the live 511 form, the injection now firing 'found 1 of 2' at the exact arm); the eighty-eighth wave closed the pre-promotion sizing family the canon-digit sweeps could not see — PA-19.4's training-audience decomposition still sized the HQ component at the 362-era '~360' beside store/DC cells already at canon (footing ~6,760 against the file's own ~6,911 Volume rows), PA-27.2's W55 DR staffing line still quoted the review-#34-era 'planned ~50 IT staff', and PA-27.3's W394 Volume still carried the 2026-06 authoring figure '~30 IT staff' (the IT canon: the 122-FTE 17-team design at ~108 active post-(ad)/(ah)) — the retired joined forms banned ('HQ staff (~360)', 'planned ~50 IT staff', '~30 IT staff') with the repaired-cell anchors required (new retired-it-sizing arm), and PA-19.5's W1383 resignation-volume cell — the derived-band class one scope layer deeper, '~60–80/month across all entities' derived from the STORE population (~5,800 ÷ 12; store-only even at its 2026-06-10 authoring) with its 720–960/year volume echoed into W1384's prorated-computations cell — re-derived at the active canon (~85–90/month ≈ 15% × 6,911 ÷ 12; ~1,020–1,080/year) with W1384's echo re-pointed to the W43 churn canon (1,200–1,600), the retired forms banned and the corrected anchors required (new retired-derived-volume arm; teeth: three synthetic injections each caught at its exact arm, fixtures restored sha256-verified byte-identical))"
else
...
echo "    $C59_N"
if [ $C59_RC -eq 0 ]; then
    ok "All model-doc tokens resolve, §-refs resolve doc-scoped, and no retired figures appear (guard mode of audit-model-docs.py, now 21 docs incl. the whole 02-oracle-ebs platform-blueprint folder, its licensing BOM, the EBS Vision instance verification report and the cross-repo quote-coverage review; and the 2026-09-21 oratest capability verification brought 02-oracle-ebs/ebs-oratest-verification.md under the guard with the oratest_hits rule — the CitiHardware R12.2.4 PROD-clone rig as the second installation of record: section skeleton, the 39-row FP × rig re-map and 10-row VT × rig matrix re-pinned, the rig tally (9 GREEN + 1 PARTIAL) re-derived from the VT verdict cells, the OR-1..OR-6 register and the rig-evidence anchors pinned every run; 3 secondary docs verified clean 2026-08-29; TO + IT operating model brought under the guard and verified clean 2026-09-02, review #68; sourcing model + technical guidelines brought under the guard with §12.1 tier-count and TO §11 phase-sum structural rules, 2026-09-03 post-AAP pass; methodology-index version pins and the two-state TO-anchor description pinned to the live docs, 2026-09-03 index-trueness pass; the OM 'Downstream:' pointer, the reality-check STATUS banner pins, and the executive-summary top-footer counts pinned to the live docs/registers, 2026-09-03 description-trueness pass; every register '(n Workflows)' heading claim and the Summary per-phase counts re-derived from the register's own rows via register_heading_hits, 2026-09-03 consistency review pass; the AI-first operating guide brought under the guard — verified clean on dry-run — with a guide_figure_hits structural rule re-deriving its catalog triple, Tier-ladder sentence and control/requirement canon-table citations from the primary registers, 2026-09-04 guard-extension pass; the OM reconciliation surfaces (§3.2 portfolio-table cross-foot, §4.9 Total row, live footer split) brought under the om_reconciliation_hits structural rule re-derived from the index Grand Total — 2026-09-05 sixth-wave review, after the batch-23 pass stranded the §4.9 Total row behind a self-satisfying stale ANCHOR; data-volumes-and-integrations.md brought under the guard with the dv_volume_hits §1.1/§1.2 arithmetic rule re-deriving every peak cell and the storage sums, the exec-summary Repository-Structure tree pinned to the on-disk top level (the bpmn/ + dmn/ rows the tree predated are now stated), and the reality-check §3.1 AP parenthetical repaired ~450/day → ~300/day — 2026-09-07 eighth-wave review; the reverse-direction companion-pin family (the sourcing model's header 'Companion to' OM pin stranded at v3.3 by the batch-20 cascade that re-pointed only its §13 sibling, the §13 OM row frozen at v3.4, the sourcing footer companion-pin chain broken by the sixth-wave OM v3.8 bump — sourcing bumped to v2.8 with its own re-point clause — and the OM §13 'this vN.M model' self-pin still at the 2026-09-03 v2.4) all re-pointed to the live footers and pinned every run by the new companion_pin_hits rule — 2026-09-07 ninth-wave review; the tenth-wave review adjudicated the sixth wave's flagged VS-151 residual (the §4.9 SSP row's enumeration claimed VS-151 while its 882 figure excluded it and DP's 213 included it — a mismatch traced to the v1.0 issuance itself) in favor of the unanimous mapping side — SSP 906 / DP 189 — and exposed the hand-maintained subtotal split ±3 off its own member rows (4,913 + 513 declared vs 4,910 + 516 derived), re-based to 4,934 + 492 with the OM bumped to v3.9 and the sourcing companion chain re-pointed (v2.9); om_reconciliation_hits extended to re-derive every §3.2 per-team cell, both subtotal cells and every §4.9 per-team workflow cell from the §4.1–4.8 mapping tables and the on-disk `##` headers each run — 2026-09-07 tenth-wave review; the 2026-09-09 thirteenth-wave review trued the v2.0 TO register issuance (the footer's '≈150 distinct roles' → the register's true 188 rows / 187 distinct titles; the §5.3 span note's 'Controller at 7 directs' → 7 sub-team lines / 8 register reporting rows in-band; the §5.2 GL & Consolidation mix cell conformed to its register rows — the same conformance class the v2.0 pass itself had applied to Store Operations and AR & Credit) and closed the guard gap with to_register_hits — §5.1's 18-department current/target columns and two-state total rows, the three §5.2 sub-team tables, every §5.3 department-table HC foot, the §5.1↔§5.3 per-department pairing, the register-total footer line, the quoted distinct-role-title count and the span-note Controller row-count re-derived every run); and the 2026-09-09 sixteenth-wave review trued three stranded-derived-figure classes — all 'the canonical table moved, the derived quote stayed' — and closed them with profile_derived_figure_hits (the revenue-per-employee quotient 9.22M → 9.21M on the profile §4 note, assumptions A2.6 and PA-133.3's Volume row after the v2.22 rebalance bumped the division's denominator 6,757 → 6,762 without re-deriving the quotient — the profile's own parenthetical division contradicted its quotient, and this module's #41 self-description had verified the stale figure; the §15.2 Master-Data trade-customer row 5,000 → 5,200 and PA-11.2's live '(5,000 accounts, ~30% of revenue)' → ~5,200 after the v2.23 canonicalization moved §9.2/§10.3 to 5,200/5,400; the rule re-derives the quotient and the account counts from the §9.4/§4/§9.2 rows, cross-foots §10.3 and §15.2 to them, and sweeps the live corpus — PA files, workflows support docs, root/methodology docs; CHANGELOG, generated trees, footers and the gap-analysis scenario table's authoring-time rows exempt — for disagreeing citations); and the 2026-09-14 nineteenth-wave review brought the whole 02-oracle-ebs platform-blueprint folder (7 documents, zero validator coverage since its same-day issues) under the guard — repairing six invented family names and two missing touchpoint-map module rows in the coverage map, the stale ASCP/Demantra 'evaluation' wording, the ~17,000-ecommerce subset-as-total NFR figure, an 'RPO/RPO' typo, the fit-gap Reading note's 6/4 BUILD-split inversion, the stale 4-component LOC budget row, the retired 'PAY-PH pack' wording, and a non-canonical CTL-1 citation — and closed the gap with ebs_blueprint_hits (fit-gap §2 register class counts re-derived onto the §3 table + intro + Reading-note split, the touchpoint-map module set and the canonical family names/counts mirrored one-for-one, the §7 ecommerce total re-derived from the profile canon, the README wave prose pinned to the classification tier ladder), with W0 exempted as a wave label and the sourcing-model + blueprint sections joining the §-resolution union; and the 2026-09-14 twenty-first-wave review closed the doctrine cascade's canonical-integration strand — the two-tier doctrine's Payroll PH → GL posting flow (fit-gap E8 INT) had never reached data-volumes §3's canonical Integration Detail Matrix, the EBS pattern register's Flow column claimed to 'quote the canonical matrix rows' while the gateway chargeback/fee row had no canonical counterpart and TWO canonical rows (Delivery Partners → ERP delivery status; Supplier Portal → ERP ASN/invoices) were silently folded into their outbound siblings, and the legacy mapping template still routed payroll YTD into the ERP against data-migration row 11 — repaired (canonical matrix row 20 + the statutory target named in full; the register's endpoints re-pointed verbatim with both folded rows split out and the chargeback row declared the register's single extension; the mapping template's §1 scope row and §2.4 YTD row re-pointed to the build) with integration_mirror_hits (the §3 matrix re-derived every run with the Payroll PH posting row required, the register's Flow sequence required equal to the canonical sequence after the one declared extension, endpoints verbatim) and migration_template_hits (corrected anchors required, the retired YTD-into-EBS routing forbidden, the 02 row-11 anchor required) inside this check); and the 2026-09-14 twenty-ninth-wave review closed the uncommitted structure-promotion session's own residue: the promotion re-based §3.3/§4/§11.1 but stranded the profile's three remaining live employee-count rows on the retired 6,762 (§11.2 Payroll-Parameters 'Total Employees', §15.2 Master-Data 'Employees', the §17 user-adoption bullet — §15.2's account rows were already cross-footed here, so the employees rows joined them, re-derived from the §4 canon every run), the same session left the OM header 'Companion to' sourcing-model pin at v3.1 when the same-day sourcing bump moved the footer to v3.2 (the ninth-wave rule pinned the sourcing→OM direction only — companion_pin_hits gains the mirror OM→sourcing arm, matched on the link-followed-by-(vN.M) form unique to the wrapped header), and the promotion's corpus sweep (which matched the ~6,762 total) never matched the HQ-subtotal family — PA-22.1's whole HQ-Departments table reconciled to the retired 362 while citing profile §3.3 as live, plus PA-72.3/PA-138.2/PA-19.3 Volume rows, PA-34.1's two laptop-refresh cells, PA-40.2's allocation step, PA-13.1's 362/5-person-trade-register note and VS-169's README intro parenthetical — all re-cut to the promoted 511 register, with the 362-class literals joining reconcile-staffing-claims' retired list (Check 51) and the CHANGELOG's own entry-letter collision (three prepended entries reusing committed letters (j)/(k)/(l)) joining Check 72; and the 2026-09-15 thirty-fifth-wave review pinned the fit-gap §7 standard-first KPI row's '(current N%)' figure to the register re-derivation — the 2026-09-15 EBS-maximization audit trued the §3 total row and the intro headline to 59/83 = 71.1% but stranded the KPI table at the retired 68.4% against its own 'This register, re-run per wave' enforcement cell, so the KPI figure now re-derives with the class counts every run; and the 2026-09-16 forty-first-wave review pinned the fit-gap register's note-join hygiene — the seventh pass's eleven register-row clauses and intro sentence shipped without their join punctuation (eight '…The 2026-09-16' unseparated runs, a ').;' intro join against the '; and the' segment convention, and the B13 note's 'pa-98.2' lowercase file reference) — appended audit clauses must now be sentence- or semicolon-separated (joined-text probe over the footer-stripped body; the forty-fifth-wave review widened the arm from lowercase-letter to letter-or-digit joins after the eighth pass's F3/F5 appends dodged it with 'A12 The'/'C14 The' digit-ended runs — repaired in place), the ').;' join and the lowercase reference retired per document, and the corrected 'PA-98.2 ST row names it' anchor required; and the 2026-09-16 forty-third-wave review added the document-map paren-balance arm — the eighth pass's append to the blueprint README's fit-gap document-map row shipped its segment-final parenthetical double-closed ('§4 resolution 32))' — the doubled-join genre one document over from the forty-first wave's fit-gap-scoped arm — so every document-map row must now carry balanced parentheses; and the 2026-09-17 fifty-third-wave review brought the 02-oracle-ebs licensing BOM (the quantitative license document the fifty-second-wave session shipped untracked and unintegrated — no document-map row, no root-README tree row, no guard coverage) under the guard with licensing_bom_hits re-deriving every scenario total from the BOM's own line items each run (per-row price×qty products and the Employee-metric HRMS rows ×6,932 — ×6,911 at issue, re-pointed by the 2026-09-18 gap-fill, the §2.10 perpetual/support/3-yr/5-yr cells, the B1/B2 monthly totals and bold TCO lines, and the §4 comparison row at its own quoted precision — teeth verified by stated-total and line-item corruptions firing at their exact arms); and the 2026-09-17 fifty-fourth-wave review extended licensing_bom_hits with the footing arm — the explanatory build-up surfaces the line-item re-derivations cannot read: §1's ecommerce driver must keep stating the derived ~4.5-lines/order basis its EOL/Pooled-line pricing rides (no lines-per-order canon exists anywhere), and §2's Financials and MSCA user build-up notes must carry their explicit footing clauses (the iExpenses 'rounded to' convention the notes had silently omitted; the MSCA DC figure trued to §1's own 600-user driver) — anchors and retired forms checked against the footer-stripped body after the arm's first draft satisfied its anchors off its own change-note (the sixth-wave anchor-satisfying-itself-off-history failure, caught by the injection teeth before shipping); and the 2026-09-17 EBS-instance verification run shipped 02-oracle-ebs/ebs-vision-verification.md under the guard with the vision_verification_hits rule — the report's section skeleton, its 39 FP footprint rows and 5 VT transactional rows (unique ids), the §2 verdict tally re-derived from the FP table's own CONFIRMED cells, the VF-1..VF-6 findings register and the instance-evidence anchors (release 12.2.12, the AR 'Revenue Management Super User' responsibility, the AMW '(Obsolete)' registry status, the Order Import wrapper) re-derived and pinned every run; and the 2026-09-18 verification round 2 re-pinned the VT table at 10 rows (the five v1.0 rows re-adjudicated — VT-1 re-confirmed, VT-5 Process_Order resolved PASS, VT-2/3/4 root-caused VF-7/8/9 — plus VT-6 requisition APPROVED, VT-7 receiving blocked, VT-8 inventory calendar-skip, VT-9 HR hire PASS, VT-10 FA mass-additions PASS) and extended the findings register to VF-1…VF-11; and the 2026-09-18 fifty-fifth-wave review swept the gap-fill's own live-surface stragglers (the TO §1 design stance still reading 'lands at HQ 511' two lines under a re-pointed states table; the classification register's live W10 significance cell still at 6,911; the licensing-BOM §1 driver breakdown still '+ 511 HQ' under a 6,932 leading figure — the sweep's 294-replacement class surviving in cells no rule read), fixed the guard's own stale two-state error-message anchors (511/6,911 → 532/6,932), brought the cross-repo quote-coverage review under the guard with quote_coverage_hits (G1–G17 and D-1–D-9 register skeletons, the §4 7,247-vs-6,932 canon row, the v1.1 post-review supersession note recording the RFQ's own v2.0.1/v2.0.2 restatement of the quoted totals) and licensing_bom_hits with the driver-row footing arm (the §1 breakdown parenthetical re-derived against its own leading figure) and gap_fill_straggler_hits (live-line 6,911 ban in the classification register with the dated blockquote/footer exemptions; the TO §1 stance anchor 'raised it to HQ 532' required and the retired form forbidden); and the same day's store-layer role-scope true-up (fix-store-role-scope.py, 93 structured-cell edits: store 'Customer Service Representative' → store-roster 'Customer Service Rep' across the 9 unambiguous store-execution files — the HQ 22-HC form was absorbing store counter/desk demand —, 'Floor Associate' role cells → Sales Associate, W562's store-daily LP routine → Store Manager per its own no-dedicated-officer Owner clause, 4 dual-scope Frequency fields re-worded per-store-first) guarded by store_scope_hits (store-file CSR form ban, Floor-Associate role-cell ban, the 9 W562 Store Manager cells required, the retired dual-scope frequency fragments forbidden); and the 2026-09-21 sixtieth-wave review closed the same gap-fill's last two straggler surfaces — the class the fifty-fifth wave named (cells the 294-replacement literal sweep could not reach) taken one level deeper to the cell that DERIVES the headcount instead of stating it: the licensing BOM's §3.2 Fusion Suite Employee tier still stood at 3,311 = 6,911 − 3,600 (the complement of the professional tier), arithmetically self-consistent with its own B2 monthly total and bold TCO chain and therefore invisible to every licensing_bom_hits arm — re-based to 3,332 = 6,932 − 3,600 with the B2 total, bold chain and §4 comparison cell re-derived (\$1,924,498 → \$1,924,603/mo; \$23.09M → \$23.10M) and the §3.2 footing clause added per the fifty-fourth-wave build-up convention; and the requirement register, whose own v24.2 footer names SEVEN employee/user-count rows while the sweep re-based only two (MDM-011, HR-036), leaving five live rows (HR-039, HSE-004, HR-041, HR-045, NFR-041) stating 6,911 employees beside two siblings stating 6,932 — one register, two headcounts — all five trued to the canon (erp-requirements v24.3); guarded by licensing_bom_hits' new Fusion employee-metric arm (every §3.1/§3.2 Employee-metric quantity equal to the §1 driver canon, the Suite Employee tier equal to canon − the professional tier, and its prose footing clause re-derived) and gap_fill_straggler_hits' new register arm (live-line 6,911 ban with the interleaved per-round version-footer lines exempt, and all seven declared rows required to carry the canon); and the 2026-09-21 sixty-first-wave review closed batch 26's own navigation-surface straggler class — the re-point cascade moved both READMEs' workflow-count surfaces but stranded the fit-gap register's own counts on the two rows no check read at register-count grain (the root-README tree row still '97-row ... (74 standard' and the blueprint README's document-map row still 'register (97 rows incl.' against the register's 103) — both re-pointed (103-row / 80 standard / 77.7%, the pass-chains gaining the H12–H17 closures clause; blueprint README v3.2) and ebs_blueprint_hits extended with the navigation-row arm (each README row naming fit-gap-analysis.md and a 'disposition register' must carry the re-derived total and, on the root tree row, the standard-total cell; the retired 97-row/(97 rows/(74 standard forms banned); and the same day's manuals-alignment sweep (by direction: check the ebs manuals, everything must align with it) verified every batch-26 product claim against the shipped R12.2 PDFs themselves (Price Protection by-item/by-warehouse covered-inventory adjustments §2-7/2-8, EAR's seeded GHG_EMISSION_FORMULAS_ALL per-operating-unit setup program and reporting key, OIC's 'CALC: Calculated > Posted' and 'PAY: Approved > Paid' phases with the Oracle Payroll pay-group integration, BPA's rule-order/'Reordering Rules' §3-7 and registered data-source views, the Financial Tax Register + Tax Reconciliation reports + reporting ledger, Copy Inventory Organization's copied items/subinventories/locators/parameters, and the Asia/Pacific guide's six-country list with zero Philippine content) — and caught one false confirmation: the coverage register's 'No EBS POS product exists in the library' was factually wrong, \`122crposig\`/\`122crposug\` (Oracle Channel Rebate and Point-of-Sale Management, 474p) being a POS-named product the library really ships — trued to the checkout sense (no store-checkout/terminal guide among 376) with the supplier-side product recorded not-needed in §5 (arithmetic restated 164 → 165 products, 32 → 33 recorded), W5579's 'assignment rules and their precedence' re-pointed to the manual's own rule-order concept (PA-16.2, two spots, online/printed orders separate), and the D6 + coverage-map POS notes qualified — guarded by ebs_doc_coverage_hits' new POS-truth arm (the §3 row must cite both crpos guides, name Channel Rebate, carry the checkout anchor; the retired bare form banned; the §5 record required) and ebs_blueprint_hits' new POS-row arm (fit-gap D6 and coverage-map POS/Retail must carry 'No EBS POS checkout product', the bare form banned on both); and the same day's sixty-second-wave review closed the batch-26 live-census class — the workflow canon moved 5,430 → 5,432 while the present-tense count cells no rule read at that grain kept the retired totals (the fit-gap intro's two census mentions, the coverage register's framing sentence, the blueprint README's §1 and §5 corpus cells, the AI-first guide's Law-2/D1 rows, the OM's §11 companion-artifacts quote, the methodology index's two tool rows, the TO's §5.3 census parenthetical, and virtual-gemba-walk.py's CANON_WORKFLOWS pin — the tool exiting 1 on every run since the batch), and the licensing BOM's §2.11 5-yr TCO shipped \$102,504,244 against its own stated components (\$102,504,245) because the chain arm re-derived it from the ×2.1 license approximation at ±1.5 tolerance — all trued (fit-gap §7's charter note also gained the post-batch-26 register state its own KPI row already stated) and guarded by gap_fill_straggler_hits' census arm (the retired 5,426/5,427/5,430 banned on the footer-stripped live bodies of the eight census documents, the 5,432 anchors required at the repaired cells, the TO's '5,432 of 5,432 owner cells' joined-form required, the tool pin and docstring required) and licensing_bom_hits' exact TCO chain (the 3-yr/5-yr cells re-derived from the stated support cell at ±0.5, retiring the approximation tolerance that ate the off-by-one); and the same day's capability sweep against the shipped R12.2 manuals (pdftotext over ebs_docs/current/acrobat/) verified the adopted-vehicle mechanics beyond the (i)/(j) products — Lockbox, Deposits and Commitments (122arug), contingent-labor rate lines billed from timecards (122poug), cXML From/To map formats (122ecxug), ECC data sources spanning iProcurement/Logistics-Inventory/Order-Management/Procurement/Projects/Receivables (122eccext), Succession Planning readiness levels and High-Potential-Worker reports (122osupiug), GL budgetary-control funds check (122glug), Site Hub attributes (122rrsug), Sourcing RFQs and auctions (122ponug), iSupplier ASNs and invoices (122posug), Project Billing retention (122pjbug), Installed Base and Service-Contracts warranties (122csiug/122oksug), Depot Repair repair orders (122csdug), TeleService escalations (122csig), iRecruitment notifications (122ircig) — and trued two vocabulary classes the manuals do not document ('L&FM rental contracts/reservations' → lease contracts with rental-period billing, rent streams and evergreen rentals per 122oklug; 'issued-but-unreleased' → the AP manual's 'issued but not matured' per 122apug), guarded by ebs_blueprint_hits' manuals-vocabulary arm (the retired forms banned on the coverage-map, fit-gap and architecture footer-stripped bodies with the fit-gap §4 resolution-record rows exempt as dated records; the 'evergreen rentals' and 'issued-but-unmatured' anchors required) and Check 46's two new workflow-corpus vocabulary literals; and the same day's sixty-third-wave review re-derived the coverage register's per-row guide citations from pdfinfo — all 23 basename-cited '/ Np' forms foot exactly as combined per-product sums, and EDC-21 held the one defect ('HRMSi Strategic Reporting (3 guides / 474p)' against a library that ships two HRMSi guides, 94p + 286p = 380p — the Deploy guide double-counted) — trued and guarded by ebs_doc_coverage_hits' pdfinfo arm (every basename-cited '/ Np' re-derived from pdfinfo, 'across k files' suffixes checked) and its EDC-21 pin (the corrected form required, the double-counted form banned); and the same day's batch-27 closures completed the coverage register (all 25 EDC rows dispositioned, 0 open — six adoptions B14/D18–D20/F7/H18, six realization trues A10/B10/C5/C10/D14/H5, thirteen not-needed products recorded in §5) with the fit-gap register re-based 103 → 109 rows (FIT-STD 44 / FIT-CFG 42, 86 standard = 78.9%; the expect-dict, total pin and §7 charter-note anchor re-pointed) and the role census's PROSE_NON_ROLE set gaining 'Oracle Customer Data Librarian' (the fit-gap D20 adopted-product-ends-in-role-head class, the H11/F3 precedent); the same day's Vision-findings re-disposition executed the two chartered VF movements (fit-gap §4 resolution 36 — H11 Oracle Internal Controls Manager re-dispositioned FIT-CFG → BUILD to the in-house Audit & GRC platform, AMW registered '(Obsolete)' with no installation record on either installation of record; the A11 vehicle naming trued to Receivables Revenue Management in-suite AR, the 'Revenue Management and Invoicing' name nowhere in the 12.2.12 registry or the shipped library) with the fit-gap expect-dict re-pointed 44/42 → 44/41 and BUILD 15 (85 standard = 78.0% of the unchanged 109; the §7 charter-note execution anchor and the Reading-note 6-new-scope split re-pointed) and Check 46's vehicle-literal guard gaining the four retired forms (Oracle Internal Controls Manager, ICM, Revenue Management and Invoicing, RM&I); and the 2026-09-22 Component-GPL true-up verified every EBS-side licensing-BOM line against the in-repo price-list PDF (applications-price-list-070574.pdf, the September 10, 2026 Component GPL): eight † part numbers filled (L72200, A92469, A81412, L98184, L10090, A85655, L42119, A80531 — three metrics trued to Expense Report/\$M Managed Assets/Compensated Individual, two placeholder prices corrected IMC \$5,750→\$25,000 and Price Protection \$2,295→\$3,000, four SKU minimums enforced), five no-SKU products (Credit Management, Oracle Quality, Engineering, Project Management, EAR) retired into §2.11's custom-quote register, Scenario A restated \$48,712,340 → \$50,228,665 with support/3-yr/5-yr following (\$11,050,306/yr; \$83,379,583; \$105,480,195) and the §4 row re-derived — guarded by licensing_bom_hits' GPL arm (no 'TBD †' form left in Scenario A, the eight part-number anchors and the no-SKU register clause required on the footer-stripped body, the arithmetic arms re-deriving the new totals each run); and the 2026-09-22 sixty-fifth-wave review trued three derived-surface stragglers of the same day's re-disposition cascade — the coverage register's §6 guard-note arithmetic quote still at the pre-batch-27 baseline (107 + 33 + 25) against §2's own restated sentence (90 adjudicated + 50 recorded + 25 opened-then-closed EDC rows of 165; the OM §11 derived-quote class — now re-derived from §2 every run so a future §2 re-statement re-fires until re-pointed), data-migration's §2 W0 sequence-tree line still routing 'the control register into ICM (24)' one section under its own re-pointed row 24, and the BOM's two Fusion Risk Management parity tags plus the §3 parity note still anchored on the retired ICM product name while the same-day VF-1 pass had re-worded the sibling Fusion Revenue Management tag to its surviving vehicle — all three guarded (ebs_doc_coverage_hits' §6-quote arm, ebs_blueprint_hits' data-migration sequence-tree arm, licensing_bom_hits' retired-parity-anchor arm; teeth: four synthetic injections through the FULL audit, each caught at its exact arm, fixtures restored sha256-verified byte-identical; and the 2026-09-23 Component-GPL sellable-SKU sweep (coverage register §7) joined the guard with gpl_sku_sweep_hits — the price-list direction: the §7 result sentence (307 priced part numbers = 55 licensed + 7 read-only-annotation + 11 prior + 234 annexed, 84 named + 150 band-priced UPK modules; 0 unexamined), the part-number universe, every backtick-cited part, the four flag pins (L11496, L72211, L31659, L42175) and the L72189 collision pin, and the UPK band count re-derived from applications-price-list-070574.pdf every run); and the 2026-09-23 sixty-sixth-wave review trued the production-volume calibration's derived-surface stragglers — the v4.7 canon moved the replenishment row to ~50,000/month, the PO row to ~12,000 lines/month at ~7 lines/PO, the §1.2 storage rows to ~123 GB/year and (pre-ecommerce-canon) the home-delivery family to the 17,200/month W19 canon, while 46 volume-citing cells across 30 documents (22 PA files, 5 VS READMEs, the classification register's W4 cell, the profile's §6.5 pair, the TO's two sizing-basis cells) no rule read kept the retired figures — the ~5,000 replenishment canon (incl. the retired 2–3-orders-per-truck consolidation wording), the ~18,000 PO lines / ~15 lines-per-PO canon (the profile's own §6.5 band left against its restated §15.1), the ~100 GB/~1,000 GB storage sizing, and the pre-ecommerce-canon ~10,000 home deliveries family — all trued and guarded by the new calibrated_volume_hits rule (the retired forms banned on live PA/README surfaces, footer-stripped bodies and the classification register's live cells; the calibrated anchors required at the repaired cells; gap-analysis scenario tables and version footers exempt as frozen history; teeth: five synthetic injections each caught at its exact arm, fixtures restored byte-identical); and the 2026-09-23 sixty-seventh-wave review extended calibrated_volume_hits for the sibling-cell stragglers the calibration and the wave both left inside the very files they repaired — W4's own Volume row + four staffing sentences (~167/day, ~42/DC/day, ~50 lines/order under a Frequency row the wave itself re-pointed), PA-04.2's other six volume-citing workflows (W1352 Frequency vs its own re-based Volume; W1353 ~33+~115; W1369 ~148; W1370 ~5,000/month vs its own re-based Background; W836's 4×~10=~40 loads/day vs its own ~1,800–2,400/month clause), PA-04.3's dock-appointment cells (30 outbound loads/DC/day vs the 60–80/day chain-wide canon), W1256's Volume (~5,000 RTP shipments vs its own re-based Background), W277/W461's freight-billable family (~10,000–17,000 home deliveries; ~12,000–20,000 sum, file-scoped banned), W919's Background tail (~5,000+ movements vs its own repaired Frequency), W784's ~60,000/year annual form, W1415's ~500–800 SKUs/delivery lines-per-drop and PA-10.2's staffing sentence — all trued to the load/drop canon (load = consolidated drop; 60–80 loads/day chain-wide = 15–20/DC/day; W4 at ~1,700 orders/day, ~3 lines/order → ~1,250 pick lines/DC/day) and anchor-pinned; teeth: three synthetic injections each caught at its exact arm, fixtures restored byte-identical); and the same day's sixty-eighth-wave review extended calibrated_volume_hits for the calibration's own written-cell drift and the new canon rows' unread derived cells — W1352's Time Estimate (written by the calibration at ~5,300/~1,060/~265 chain-wide pick lines against its own ~1,250/DC/day Frequency anchor → ~5,000; hours re-footed), W92's adjustment cells at ~1,700–2,600/month against the §1.1 ~30/day IAD row (re-based to 200×~3–5 + 4×~20–30 = ~680–1,120/month), W270's RTP cell at ~40 receipts/day/DC against PA-04.1's canon-citing ~50, W3962's retired ~1,400–1,600 POs/month band (trued to ~1,200 merchandise POs/month, all types ~1,600–1,900), PA-28.2's two remaining 1,000 GB cells against its own ~1,230 GB anchor, and W109/W666's 20–50-lines-per-delivery cells against the 60–90-line consolidated-drop canon — with the wave-66 '5,000 replenishment' banned form narrowed to '5,000 replenishment orders' after it fired on the corrected anchor; teeth: four synthetic injections each caught at its exact arm, fixtures restored sha256-verified byte-identical; and the same day's sixty-ninth-wave review trued the calibration straggler one layer further out — the root README's at-a-glance PO row (the one volume-citing surface neither the calibration cascade nor waves 66–68 swept — the README sits outside the PA/README glob and outside the versioned-doc list) still carrying the retired ~1,400–1,600 total-PO band the sixty-sixth wave retired on profile §6.5/TO/W2 and the sixty-eighth wave retired on W3962 — trued to the §15.1 canon (~1,200 merchandise; ~1,600–1,900 total production-calibrated) with calibrated_volume_hits gaining the root-README arm (the retired '~1,400–1,600 total' form banned and the calibrated anchor required; the loose numeral pair deliberately not banned repo-wide — PA-79.2's separated-employee cells legitimately use it); and the same review moved the §4-resolution-37 Asset-Tracking entitlement flag (L11496) from the D6 POS row — a paste-family misdirection, the flag qualifying F7's own rides-the-bases clause — to the F7 row its resolution record declares ('carried on the F7/B10 note cells'), guarded by ebs_blueprint_hits' new flag-placement arm (the entitlement-flag/L11496 forms banned on the D6 row, the flag anchor required on the F7 row); teeth: four synthetic injections through the FULL guard, each caught at its exact arm with file and line named, fixtures restored sha256-verified byte-identical; and the same day's seventieth-wave review trued the receipt/ASN/container sibling family — one derivation step out from the pinned cells: W3's '~15 lines per receipt' Volume (PA-04.1 — the retired ~15-lines-per-PO basis the sixty-sixth wave retired on the PO surfaces, which × the ~6,000 receipts/month canon implied ~90,000 receipt lines/month against the ~12,000 PO-lines/month all-types canon; trued to ~2 lines per receipt — bulk POs arrive as multiple partial truck receipts), W3C's '~20–30 import containers per DC per month' against W1271's own '~100–150 containers/DC/month' and W1169's '~20–30 per DC per week' (both deriving from the ~400–600 TEUs/month §7.1 import canon at ~1 TEU/container; trued to ~100–150, PA-04.1-file-scoped banned — PA-01.1's per-seasonal-event forward-buy form exempt), W1365's ASN universe ('~4,800 ASN submissions/month' was 80% × the 6,000-receipt canon against W1006's 1-ASN-per-PO ~700–750/month and PA-03.4's ~5,000–10,000 combined portal-document band, and its own 15–20% exceptions of 4,800 = 720–960 equaled W1006's ENTIRE ASN universe — nearly every ASN discrepant against its own <70%-accuracy flagging threshold; trued to ~700–750 ASNs with ~105–150 exceptions and the hours/staffing re-footed ~18–50 h/month chain-wide, ~4–13 h/DC), W1006's 'average 15 lines per ASN' under its own 1-ASN-per-PO model (trued to ~7 lines per ASN at the 6.9–7.4 lines/PO canon), and W1169's '~15–25 container drayage moves per DC per week' below its own Frequency row's ~20–30 containers/DC/week arrival canon (trued to ~20–30 round-trip moves, one per container) — all anchor-pinned in calibrated_volume_hits' seventieth-wave arms with the file-scoped container ban; teeth: five synthetic injections through the FULL audit, each caught at its exact arm with file and line named, fixtures restored sha256-verified byte-identical); and the same day's seventy-first-wave review trued the returns canon — the one §1.1 row the calibration grounded but whose workflow family it never re-derived: the measured Customer-Returns row (14,744/13,584 AR credit memos/year at ~1.05 lines/doc → ~40/day ≈ ~1,200/month) sits 47x below the workflows' pre-calibration ~56,000/month (2% × 2.8M transactions) assumption, and the ecommerce sub-canon (~4,000–5,000 requests/month at ~10–12% of ~42,900 orders) is untenable inside a ~1,200/month all-channel universe (W12B's cell even miscited '≈110–140/day per the corpus returns canon' for a SUBSET of a 40/day canon) — ~50 cells across 16 documents re-derived (W1622 ~950–1,050 counter returns = the canon less W1629's ~100–150 B2B credit notes and W215's ~50–100 courier pickups; W1623 ~250–350 at ~0.6–0.8%; W12B/W1273 ~200–250 drop-offs; W1624 ~325 exchanges; W1625/W1626 ~180–215 each; W1627 ~875 refunds; W1629 ~100–150 B2B at ~2–4% of the ~3,200–5,200 B2B order universe; W509 ~500–1,000 items; W540 ~1,200 BIR credit notes; W605/W1336 ~36–60/~60–95 fraudulent; PA-32.3 ~360 exceptions; W707 ~40 total returns/day; W101 ~1,300–1,550; the profile §5 bullet ~0.04% (v3.4); custody E-09 (v1.11); GOV-051 (v24.4); PA-30.2/PA-80.2/PA-118.2/PA-125.2/PA-149.3/PA-157.2 rate cells; hours and staffing re-footed throughout, all absorbed, no HC change; category-rate prose and the '~56,000 hours' training population exempt) with calibrated_volume_hits' seventy-first-wave arms (the retired forms banned on live surfaces, the corrected anchors required at the repaired cells, the custody register's live body swept with the E-09 anchor required); and the same day's seventy-second-wave review trued the dodging-variant stragglers — the three families the prior sweeps' banned forms read past at the glyph level: the hyphenated '~5,000 store-replenishment orders/month' — PA-136.1's W4126 Volume + the VS-136 README framing sentence, both wave-66 banned forms being space-form — with W4126 step 2's surviving 'replenishment grouping 2–3 orders/truck' consolidation mechanic, the returns-canon sibling stragglers inside the very files the seventy-first wave repaired — PA-32.3's W1641 '~56,000 return dispositions/month' Frequency + '~70% of ~56,000 monthly returns' touchpoint + 'mis-routes thousands of returns' pain point against its own wave-71-trued ~1,200/month Time Estimate, W1640's refurb pool at ~500–800 items/month = 42–67% of ALL returns re-based to the ~180–300 damaged-packaging-functional share at ~45–75/DC with hours re-footed ~900–1,500 h/yr, PA-23.3's W1336 Background still at '~56,000 product returns per month per W12/W1627' with 'PHP 60–100M in annual losses' derived from the retired fraud universe — trued to the ~1,200 canon with ~PHP 3–6M/yr at 720–1,140 fraudulent returns × ~PHP 3,500–5,000 avg — and step 1(f)'s '>5% of transactions (~2.5x the chain average)' on the retired ~2% basis, trued to the ~0.04% canon — and the AP business-day family review #60 left behind: #60 trued W7's staffing headline to 395–440 but W7's own Time Estimate still derived from the retired 305–370 band (~25–31 h scanning, '~70 invoices × 25 min' = ~25–30 h exceptions, total ~60–70 h) with its staffing tail still summing to '~53–67 h / ~5–6 hours each', and PA-22.1's copy of the band survived verbatim; all trued — scanning ~33–37 h, exceptions ~33–37 h at 79–88 invoices, total ~68–76 h, 10 clerks ~7 h each — and guarded by calibrated_volume_hits' seventy-second-wave arms: the hyphenated form, the 56,000-dispositions/monthly-returns/product-returns forms, the consolidation-mechanic form, the refurb-pool and W1336 forms, and the 305–370 band with its derived hour cells banned, the corrected anchors required at the repaired cells; teeth: four synthetic injections through the FULL audit, each caught at its exact arm with file and line named, fixtures restored sha256-verified byte-identical; and the same day's directed virtual-gemba cadence-ladder fix (by direction: implement the recommended permanent fix from the post-calibration gemba & time-and-motion re-run) — the production-volume calibration's compound 'chain-wide N–M per period (n–m per store per period)' Frequencies (6 cells corpus-wide, e.g. PA-56.2's W2201) had the tool's rule-1/rule-2 phrase bridges crossing the parenthesis, reading W2201 at 21.84M events/yr (840x its true ~26,000; the re-run's receiving-clerk ×12.9 / store-manager ×2 demand jumps were this artifact, not workload — corrected, receiving clerk 101% → 102% and hot/cold membership unchanged pre/post calibration, the calibration itself demand-neutral) — gap_fill_straggler_hits gains the cadence-ladder arm: the paren-crossing bridge class banned in virtual-gemba-walk.py, the paren-blocked class required exactly 4×, the docstring measurement-assumption bullet pinned; teeth: synthetic injection re-minting the retired bridge caught at the arm, fixture restored sha256-verified byte-identical; and the same day's seventy-fifth-wave review completed the Atlassian incorporation's own completeness — Atlassian Guard named as the suite's centralized identity plane (A6.6 row, Design-Decisions row, guardrails 5/7), the guardrail 2/4 built-product scoping with the EBS ADOP online-patching boundary, Bitbucket branch permissions pinned, the two EBS-adjacent suite flows joining data-volumes §3 (v4.8) with the EBS pattern register mirroring (integrations.md v1.3), the sourcing model §8 naming the suite with the §12.2 'in capability sourcing' true-up (sourcing v3.9, OM companion v3.19, headcount banner re-pointed), and the A6.6 Source cell completed to all 16 PA-27.1 workflows — guarded by the new atlassian_suite_hits rule (anchors required at every repaired surface — a future Atlassian re-architecture re-fires them until consciously re-pointed, the Check-71 CENSUS-pin contract; the retired universal-chain and Guard-less SIEM forms banned doc-scoped; the incorporation pass's 'Jira/ServiceNow'/'TestRail'/'(Confluence, SharePoint, or ITSM built-in KB)' hedges banned repo-wide across the authored trees, the methodology-index row exempt where it quotes the literals in its own guard description, the Check-46/wave-9 convention), whose first live run caught the methodology-index OM row still at v3.18 pre-ship; and the same day's seventy-sixth-wave review trued the batch-30 census stragglers on the surfaces no arm read — the value-stream-index H1 banner and closing Total footer (both maintained live at batch 26, both stranded at 5,432), the conformance canon's own §1 framing sentence (canon bumped v1.5 with the conformance guard's DOC anchor re-pointed), the IT domain companion's Technology-&-Data roster (393 → 394 / VS-113 (31) → (32), Check 76's own class one derivation step outside its four quoted cells), and the retired 6,762 employee canon on the IT/operations/people companions' §1 scope lines and evidence cells (the twenty-ninth-wave headcount sweep covered the PA files and VS READMEs but never opened the four domain companions) — guarded by the new domain_companion_hits rule (retired forms banned doc-scoped, the corrected anchors required at the repaired cells — the Check-71 CENSUS-pin contract); and the same day's seventy-seventh-wave review trued the (x) trade-desk disablement's own residue — the three population cells its 252-cell sweep's surfaces did not reach, plus the quotient arm's own newly-legitimate false-pass: the blueprint README §4 W4 People row's retired '6,932 employees' (the file sits outside the PA/VS-README sweep; ebs_blueprint_hits arm (l): retired form banned on the footer-stripped body, the '6,925 employees' active anchor required at the W4 row), the AI-first guide §1.4 reference-parameters cell's 'HQ 532 … 6,932 employees' with the 09-18 edit's mangled '— was 2026-09-14' fragment (guide_figure_hits arm (f): both retired forms banned, the 'HQ 525 active' / '6,925 employees (TO design 6,932)' anchors required; §6.6's dated promotion arrows exempt as frozen history), and PA-133.3's half-repaired W4067 Volume cell ('~6,925 employees … target ~PHP 8.99M') which PASSED the quotient sweep every run because the (x) settlement made the TO two-state row's design column (6,932) compute to the same 8.99 the retired active canon produced — profile_derived_figure_hits' two-base tolerance scoped per surface kind (PA/VS-README citations require the active canon, companion surfaces may still cite the design quotient); teeth: three synthetic injections through the FULL audit, each caught at its exact arm with file and line named, fixtures restored sha256-verified byte-identical; and the same day's seventy-eighth-wave review brought the Online Channel & Capability Registry itself under the guard (channel-capability-registry.md joins the DOCS set at 22 documents — it had carried only the (x) trade-estate anchor arm since the disablement, the licensing_bom/oratest one-wave-late precedent — with the new channel_registry_hits rule re-deriving the §3 tables' state census and CAP-family set every run and asserting W5580's fields against them, plus the TO joining the bare-§ union for the registry's 'TO §5.2/§5.3' register cites) and trued the four switchboard-residue defects the (x) B2B estate left on the no-arm-read surfaces: W5580's Trigger row-family enumeration now covers CAP-Bxx (registry rule 7 routes the B2B estate's state changes through W5580), its Volume census re-based 9 → 13 prepared-disabled (the batch-30 count plus CAP-B01–B04), its step-8 comms citation re-pointed from the price-change workflow (the retired 'W518/W40' form) to the W2358 corporate-announcement canon (VS-63), and registry rule 7's VS-11/VS-43/VS-107/VS-68 estate re-termed from 'process areas' to 'value streams' (the corpus's own 188-VS/569-PA convention; registry bumped v1.3); and the seventy-ninth wave trued the (x) active-canon sweep's own HQ-level residue — live population cells still sizing off the retired HQ-active 532 beside neighbors the same sweep had re-based to 6,925 (the half-repaired-cell class: PA-19.3's eligible population, PA-40.2's allocation HQ component, PA-138.2's canteen Volume, PA-72.3's shared-services Volume, PA-34.1's laptop pair, PA-22.1's functions-table preamble + reconciliation recital, PA-13.1's trade-desk staffing bullet, VS-169's README population decomposition), the profile §4 HQ-row two-canon + footing arm added to profile_derived_figure_hits, licensing_bom_hits gaining the (i) 650-filer iExpenses arm, and the new tg_hq_bandwidth_hits rule pinning technical-guidelines §2.2's HQ WAN-sizing row to the active canon at its own ~90%-ratio precedent; and the eightieth wave bannered the (x) B2B estate's own navigation surfaces — the four value-stream READMEs registry rule 7 names (VS-11/VS-43/VS-107/VS-68) carried live revenue/account claims with no state annotation while every other disabled capability's surfaces carry the registry banner (the rule-2 PA-10.2 convention; VS-93's CAP-F08 / VS-65's CAP-C03 precedents) — banners added and the new b2b_estate_hits rule deriving the README set from rule 7's enumeration every run, requiring the banner class, registry link, designed-capacity anchor, W5580 and the per-VS CAP pin (a future estate change re-fires until consciously re-pointed, the Check-71 CENSUS-pin contract); the 2026-09-23 (ad) TPS-squad deferral re-pointed the arm literals to the post-(ad) canon — 6,918 active / HQ 518 / ≈466 concurrent / 643 filers / ×6,918 chains / quotient ~PHP 9.01M — with the retired 6,925 / 525-active / ≈473 / 650-filer forms joining the ban lists (re-points of existing arms, no new arms); and the eighty-third wave closed the (ad)-cascade's own residue — five half-repaired cells on live, unguarded surfaces, each beside a cell a prior cascade had touched or re-pointed: the OM §9.2 Industry-benchmark band (100–168 → ~104–173 — a 6,715-era derivation carried through three headcount regimes while its parenthetical was re-pointed twice; the new om_benchmark_hits derives the band from the canon constant so a census move re-fires it), the assumptions A6.6 seat-avoidance note (~6,925 → ~6,918-seat; atlassian_suite_hits arm), the registry rule-7 headcount parenthetical recut to the two-event lineage plus the CAP-B01 checklist endpoints re-based (518 → 525 on re-enablement; the 525 → 532 hop pinned to the sourcing register's trigger; channel_registry_hits arms), and the sourcing §13 OM row's 're-based 6,925' clause trued to 6,918 (companion_pin_hits arm); teeth: four synthetic injections through the FULL audit, each caught at its exact arm with file and line named, fixtures restored sha256-verified byte-identical; and the eighty-fourth wave closed the (x)/(ad) estate's own chain-level navigation residue — the retired 'all its capabilities and systems fully enabled' state claim annotated per the registry's state discipline on the root README and executive-summary intros (the wave-80 class one level up: both documents' own data rows already carried the state), the profile §14.1's TPS landscape row re-cut 'Live — Built In-House (TPS)' → Deferred — prepared with the Field-Service-dispatch seam clause (the exec-summary sibling the (ad) pass itself re-cut — the new estate_state_intro_hits rule pins all four surfaces), and the AI-first guide's §13 OM navigation row recut to the two-state form (16 teams / 115 FTE active of the 122-design; guide v1.18, profile v3.10, OM v3.27 companion re-point, sourcing v3.17; banner segment appended), teeth: four synthetic injections through the FULL audit, each caught at its exact arm with file and line named, fixtures restored sha256-verified byte-identical; and the eighty-fifth wave trued the (ad)/(x) cascades' three remaining residues — the licensing BOM v2.12 change-record's own chain recital (a +\$5 slip at the §2.8 license line — the nine Employee-metric rows sum to \$5,845,710 — compounded through the four totals it quoted, so the change-record disagreed with the body it documents; the five figures corrected in place, BOM bumped v2.13; licensing_bom_hits arm (j): any 7+-digit dollar figure in the NEWEST footer segment within \$100 of a re-derived chain value without equaling it is a near-miss recital), the TPS project-estate's own navigation surfaces (the sourcing register §4 row holds VS-74/VS-77's project-side workload dormant under CAP-B01 with VS-143 exempt per the (ad) seam ruling, but the two READMEs carried zero state annotation — the wave-80 class one record-of-record over; banners added per the VS-93 template and the new tps_project_estate_hits rule derives the VS set from the register row every run, requires the banner class/sourcing-register link/registry link/designed-capacity anchor/W5580 on each project-side README and protects the seam VS from banner classes), and the seven single-deferral design annotations completed to the canonical two-deferral form (each explained 7 of the 14 gap: 6,932 − 6,918 = the Trade department's 7 + the TPS build squad's 7; PA-138.2/PA-34.1/PA-40.2/PA-72.3/PA-19.3/PA-13.1 Volume-and-step cells + the blueprint README W4 row, README bumped v3.10; ebs_blueprint_hits arm (l2) requires the two-deferral form at the W4 row and bans the single-deferral form, reconcile-staffing-claims gains the anchors + the retired-single-deferral bans; teeth: four synthetic injections through the FULL audits, each caught at its exact arm with file and line named, fixtures restored sha256-verified byte-identical; and the same day's (ah) OMO-squad deferral re-pointed the arm literals to the post-(ah) canon — 6,911 active / HQ 511 / ≈460 concurrent / 636 filers / ×6,911 chains / quotient ~PHP 9.01M ÷ 6,911 — with the retired 6,918 / 518-active / ≈466 / 643-filer forms joining the ban lists, the W4 row's annotation extended to the three-deferral form (6,932 − 6,911 = 7 trade + 7 TPS + 7 OMO), and the licensing-bom approximation arms subordinated to the exact chain arm (the 22%-of-license support cell non-integral at the new canon — re-points of existing arms, no new arms; and the eighty-sixth wave closed the (ah) cascade's own switchboard/PA residue — the registry §3.4 CAP-B01 enable-checklist's headcount cell still taught the (ad)-era endpoints ('HQ 518 → 525 active; the TPS build squad's 525 → 532 hop … (ad)'), the very cell the eighty-third wave had repaired, skipped by the (ah) bump that re-pointed rule 7's own lineage (the recurring half-repaired-cell class on the switchboard): recut to the three-hop walk at the current HQ-active 511 (511 → 518 on CAP-B01 re-enablement; the TPS 518 → 525 hop and the OMO 525 → 532 hop each riding their sourcing-register stand-up triggers), registry bumped v1.5 → v1.6, and the channel_registry_hits checklist arm re-pointed with the retired (ad)-era form joining the bans; the guard's own stale self-descriptions trued (the wave-77 stale-comment class one level deeper — the messages the arms print on fire: this check's guide-arm 'active canon: 6,918 employees' message and the channel_registry ban-why 'now moves HQ 518 → 525', plus reconcile-staffing's retired-HQ-active ('HQ 518 active … (ad)') and retired-single-deferral ('the active canon is 6,918/HQ 518 … two-deferral required') hit-messages — all re-pointed to the post-(ah) canon 6,911/HQ 511/three-deferral); PA-22.1's functions-table reconciliation recital trued (its (x)-disablement endpoint read 'active HQ 518' where the (x) canon is 525 — 518 is the (ad) value, the recital's own final clause correctly ending at 511; 'active HQ 518' joins the retired-HQ-active bans with the corrected anchor required) and PA-30.3's half-repaired Annual-estimate summary line trued to its own re-based derivation (~13,836 → ~13,822 beside its own '×6,911 = ~13,822' line — hours, not headcount, invisible to the headcount arms; the retired derived-figure arm bans '~13,836' on PA/README lines and requires the corrected anchor); teeth: three synthetic injections through the FULL audits, each caught at its exact arm with file and line named (the registry re-mint firing both channel_registry arms at :65, the PA-30.3 summary re-mint firing both new derived-figure arms at :551, the PA-22.1 recital re-mint firing both retired-hq-active arms at :494), fixtures restored sha256-verified byte-identical; eighty-second-wave review (2026-09-23): pa_version_receipt_hits added — live companion-version receipts inside PA workflow files (no footers there, so every token is live prose) must equal the cited doc's own live footer version, the PA-22.1 IT-sizing cell's '(…; OM v3.28)' receipt found two OM generations stale and re-pointed to v3.31, the arm re-derived per run so any future companion bump re-fires it)"
else
...
# Since the 2026-09-16 forty-seventh-wave review it also byte-verifies the shipped
# bpmn/ and dmn/ trees themselves via the generators' new --check modes: the
# seventeenth-wave content mirror re-derives only each process's documentation/
# start-event-name/controls-annotation (the batch-24 class it was built for), and
# the DMN tree had no content pin at all — so a PA edit shipped without
# regeneration moved nothing this check read whenever only task names, lane names,
# sequence-flow wiring or decision content changed (the forty-sixth wave's
# hand-regeneration of PA-09.2 after its quote repairs is the live example of the
# discipline the arm now enforces). --check re-derives every file in memory and
# byte-compares: stale content, missing files, hand-edited files, partial
# regenerations and stale extras all fail at their exact file.
...
# semantically-mangled Owner cells rewritten in their PA files): ALL 5,432
...
case "$C71_CENSUS_OUT" in
  *"CENSUS workflows=5433 owner_resolved=5433 owner_uncharted=0 owner_uncharted_forms=0 uncharted_forms=0 ctl_owner_cells=808 ctl_owner_resolved=808 ctl_owner_uncharted=0 ctl_owner_uncharted_forms=0 prose_role_uncharted=0 prose_role_forms=0 anchor_chartered=259 anchor_anchored=259 anchor_zero=0 anchor_weak=53"*) C71_CENSUS_OK=1;;
esac
...
# byte-verifies the third generated artifact, and the seventeenth-wave content
# mirror byte-verifies three BPMN fields, but the shipped trees' remaining
# content — every task name and lane set (step Activity / Role (R) projections),
# the sequence-flow wiring, and the ENTIRE dmn/ content layer (decision names,
# rule entries, FEEL bands) — was pinned by nothing: structural validation reads
# shape, not text. A PA edit shipped without regeneration, a hand-edit of a
# generated file, a partial regeneration, or a stale file whose PA no longer
# derives it could therefore ship with this check green. Both generators now
# carry a --check mode (render in memory, byte-compare, never write) and the
# validator invokes them every run — the same shipped-currency guarantee the
# role-coverage matrix has had since the thirtieth wave.
C71_BPMN_CHK_OUT=$(python3 "$REPO_ROOT/07-methodology/generate-bpmn.py" --check 2>&1) && C71_BPMN_CHK_RC=0 || C71_BPMN_CHK_RC=$?
C71_DMN_CHK_OUT=$(python3 "$REPO_ROOT/07-methodology/generate-dmn.py" --check 2>&1) && C71_DMN_CHK_RC=0 || C71_DMN_CHK_RC=$?
if [ "${C71_BAD:-1}" -eq 0 ] && [ "$C71_RC_RC" -eq 0 ] && [ "$C71_CENSUS_RC" -eq 0 ] && [ "$C71_CENSUS_OK" -eq 1 ] \
   && [ "$C71_BPMN_CHK_RC" -eq 0 ] && [ "$C71_DMN_CHK_RC" -eq 0 ]; then
    ...
    ok "Generated trees validate structurally against the markdown corpus AND mirror the generator's content derivation: bpmn/ $(echo $B71 | cut -d' ' -f1) files / $(echo $B71 | cut -d' ' -f2) processes (one per confirmed-register row) and dmn/ $(echo $D71 | cut -d' ' -f1) files / $(echo $D71 | cut -d' ' -f2) decisions — well-formed XML, 1 start/1 end per process, full lane coverage, 1:1 diagram:plane, complete DI shapes/edges/bounds/waypoints, decision-table structure, DRD shape per decision, and every process's documentation/start-event-name/controls-annotation byte-equal to the generator's re-derivation from its PA markdown (content mirror added by the 2026-09-10 seventeenth-wave review after batch-24 shipped PA-133.1/.3's generated files stale at the retired 5,426 — structural counts were all still correct, so nothing else could see it), and the shipped role-coverage matrix byte-identical to generate-role-coverage.py --check's re-derivation from the PA RACI fields + tier register + official TO (third-generated-artifact arm added by the 2026-09-15 thirtieth-wave review — the matrix's only harness had been the generator's own --check, which the validator never ran), and the role-vocabulary census pinned at the adjudicated baseline (owner cells 5,432 resolved / 0 uncharted, and ZERO uncharted forms across ALL FOUR RACI surfaces — full accountability AND involved-role consistency: the thirty-second-wave reconciliation resolved every Owner cell, and the thirty-sixth-wave reconciliation (2026-09-15, by direction) extended the same standard to the participant/step-prose population — 3,253 distinct previously-uncharted actor forms adjudicated to register titles, roster roles, IT seats, department grain, or their governance/system/workforce/external bucket, driving the matrix to 4,199 resolved actors / 0 uncharted after the forty-fifth-wave review re-seated the three vehicle-naming role cells (PA-110.2 'Logistics / TMS' → Logistics, PA-110.3 'AP / Logistics / TMS' → AP / Logistics, PA-192.2 'Dispatcher / TMS' → Dispatcher) and retired the census's TMS system-actor form (4,200 after the eighth-pass review retired the 'Lms Vendor Account Manager' seat; 4,201 at the wave-36 baseline); census arm added by the 2026-09-15 thirty-first-wave review so a batch that edits owners, aliases or the register and regenerates consistently still cannot move the resolution population silently — any new vocabulary a future batch mints re-fires this pin; the thirty-eighth-wave review (2026-09-16) extended the census to the internal-controls-matrix's Owner column — the fifth adjudicated surface, the register behind the 808 CTL rows, whose 80 uncharted authoring-time function-owner forms the waves-32/36 reconciliations had never read — each adjudicated via the resolver's wave-38 alias table to the real org actor owning the workflows the control row cites, with the pin extended to ctl_owner_cells=808 resolved / 0 uncharted; the thirty-ninth-wave review (2026-09-16, by direction) extended the census to the step-Activity & analysis-field prose — the sixth adjudicated surface, swept by a frozen candidate extractor (paren-qualifier strip, ≥1-qualifier-word role spans, noise-prefix and non-role-span filters) resolving every candidate through the same semantics as the RACI cells — 46 uncharted prose instances across 25 forms adjudicated: plural/prefix/abbreviation variants aliased to the chartered titles their own workflow's Role (R)/(A) cells carry, external licensed practitioners and government officials bucketed external, appointed emergency/stewardship functions bucketed governance, and 19 PA files' prose re-pointed to the org of record (the bare 'X Specialist' mentions wrapped to the chartered 'Sales Associate (X Specialist)' form, the unchartered 'Duty Manager'/'Lead Cashiers'/'HQ Director' seats re-pointed to the chartered ASMs and Department Heads, 'Installation Service Manager' to the W600 canon) — the pin extended to prose_role_uncharted=0 prose_role_forms=0; and the shipped trees themselves byte-verified every run via the generators' new --check modes (forty-seventh-wave review, 2026-09-16 — full-tree in-memory re-derivation and byte-compare, no writes, closing the surfaces the seventeenth-wave content mirror never read: task names, lane names, sequence-flow wiring and the entire dmn/ content layer, which structural validation reads only as shape — a PA edit shipped without regeneration, a hand-edit of a generated file, a partial regeneration, or a stale file whose PA no longer derives it now fails at its exact file; the forty-sixth wave's hand-regeneration of PA-09.2 after its quote repairs is the live example of the discipline the arm enforces)"
else
    ...
    if [ "$C71_BPMN_CHK_RC" -ne 0 ]; then
        echo "shipped bpmn/ tree stale vs its markdown source (generate-bpmn.py --check — regenerate):"
        echo "$C71_BPMN_CHK_OUT" | sed 's/^/    /'
    fi
    if [ "$C71_DMN_CHK_RC" -ne 0 ]; then
        echo "shipped dmn/ tree stale vs its markdown source (generate-dmn.py --check — regenerate):"
        echo "$C71_DMN_CHK_OUT" | sed 's/^/    /'
    fi
fi
...
# 2026-09-16 forty-third-wave addition — footer/history-block paren completion:
# every '*Date:' / '*Document Version:' footer block repo-wide (block = the footer
# line plus its wrapped continuation lines, blank-line-delimited) must be
# paren-balanced after masking code spans and short quoted paren-literals (the
# '(v1.0'-style cited pin forms are content). The wave found eleven Prior-segment
# openers '(consistency review #NN:' / '(vNN —' that their segments never closed
# (methodology-index README ×6, requirement-workflow-matrix ×5) plus ten in the
# workflow-format-guide — the file's own closed segments (e.g. '…warnings).') prove
# the convention — and no rule read them. Only unclosed openers fire: extra closes
# in quoted repaired-literal citations ('…').;'') stand per the fit-gap footer's
# own adjudication, and doubled-close classes carry their own guards
# (ebs_blueprint_hits document-map arm; fit-gap join-hygiene arm).
for dirpath, dirnames, filenames in os.walk(ROOT):
    dirnames[:] = [d for d in dirnames if d not in ('.git', 'bpmn', 'dmn', '__pycache__')]
    for fn in filenames:
        if not fn.endswith('.md') or fn.startswith('CHANGELOG'):
        ...
        f = os.path.join(dirpath, fn)
        rel = os.path.relpath(f, ROOT)
        lines2 = open(f, encoding='utf-8').read().split('\n')
        for i2, l2 in enumerate(lines2):
            if not (l2.startswith('*Date: ') or l2.startswith('*Document Version: ')):
                continue
            end2 = i2
            while end2 + 1 < len(lines2) and lines2[end2 + 1].strip():
                end2 += 1
            blk = '\n'.join(lines2[i2:end2 + 1])
            blk = re.sub(r'`[^`]*`', '', blk)
            blk = re.sub(r"'\([^']{0,14}'", "", blk)  # short quoted paren-literals
            depth2 = 0
            for ch2 in blk:
                if ch2 == '(':
                    depth2 += 1
                    ...
                    depth2 -= 1
            if depth2 > 0:
                bad.append(f"{rel}:{i2 + 1}: footer/history block leaves {depth2} "
                           f"paren opener(s) unclosed — every 'Prior'-segment "
                           f"parenthetical must close at its own segment end "
                           f"(forty-third-wave arm)")
...
# TOTALS prints after every arm appends — the count-blind placement this heredoc
# shipped with (TOTALS before the CHANGELOG-letter block) made the twenty-ninth
# wave's letter-collision arm toothless in the pass/fail count — a synthetic
# duplicate letter passed the full validator 0/0 until the 2026-09-16
# forty-third-wave review moved the print below all appends.
...
if [ "${C72_BAD:-1}" -eq 0 ]; then
    ...
    ok "All $V72 versioned document footers carry a strictly decreasing Prior chain below the live version (guard added by the 2026-09-07 eleventh-wave consistency review — the AI-first operating guide had shipped all eight batch-16\u201323 re-point clauses labeled v1.1\u2013v1.8 beneath a version header that never left '1.0 | Initial issue', an inversion invisible to the index-pin rule because the pin satisfied itself off the same stale footer; forty-third-wave extension, 2026-09-16: every '*Date:'/'*Document Version:' footer block is also paren-balanced — unclosed Prior-segment openers fire, short quoted paren-literals and code spans masked — after the wave found 21 unclosed '(consistency review #NN:' / '(vNN —' segment openers across the methodology-index README (6), requirement-workflow-matrix (5) and workflow-format-guide (10), repaired in place; and the heredoc's TOTALS print moved below all arm appends, repairing the count-blindness that had let the twenty-ninth wave's CHANGELOG letter-collision arm pass a synthetic duplicate letter 0/0)"
else
...
# fifty-fourth-wave review (2026-09-17) — portability arm: the eighteenth-wave
# shipped-tooling repair re-pointed ten scripts from hardcoded '/home/<user>'
# absolute repo paths to repo-relative resolution, but the fifty-second-wave
# straggler scripts re-introduced the class ('/home/alden/erpplans_ebs') and the
# fifty-third wave Contents-rowed them without running them. Ban the functional
# form across the .py tools; audit-model-docs.py's docstring narrative quotes the
# bare '/home/riddler'/'/home/alden' strings without the repo suffix and stays legal.
for fn in sorted(disk):
    if not fn.endswith('.py'):
        continue
    src = open(os.path.join(meth_dir, fn), encoding='utf-8').read()
    for m in re.finditer(r'/home/[A-Za-z0-9_.-]+/erpplans_ebs', src):
        bad.append(f"07-methodology/{fn} resolves its repo root by hardcoded absolute path "
                   f"'{m.group(0)}' — use dirname(dirname(abspath(__file__))) (the "
                   f"eighteenth-wave shipped-tooling portability class)")
...
echo "" 
echo "--- Check 80: Generated-analysis artifact harnesses ---"
# --- Check 80: Generated-analysis artifact harnesses (gap report, weak-anchor verification, semantic detectors) ---
# Three shipped analysis artifacts/tools had no in-validator harness — the exact
# class the thirtieth-wave review closed for the role-coverage matrix ("the
# matrix's only harness had been the generator's own --check, which the validator
# never ran") and the eighty-first wave closed for the conformance guard ("run
# manually beside the validator by every wave until then"):
#   * role-coverage-gap-analysis.md — generated; its only harness,
#     role-coverage-gap-analysis.py --check, existed but was never invoked by
#     validate-repo.sh (0 mentions) — a hand-edit or a missed regeneration after
#     an anchoring/PA change shipped silently;
#   * weak-anchor-demand-verification.md — generated by verify-weak-anchor-
#     demand.py, which had NO --check mode at all; every consistency wave
#     verified the shipped report by a manual no-op regeneration + diff (the
#     CHANGELOG battery lines). The 2026-09-23 eighty-second-wave consistency
#     review (by direction: review everything; implement recommendations) added
#     the --check mode (in-memory re-derivation + byte-compare; the flag is
#     captured at module level because vgw.load_grc() rebinds sys.argv — the
#     MOTION_FULL lesson) and wired both harnesses here;
#   * final-semantic-coverage.py — the Check-62 closure detector suite (D1–D4),
#     run manually beside the validator by every wave since the closure
#     ("both modes 0 flags"); its --all --quiet regression net (exit-code
#     gated) joins the same check.
#   * manhours-rollup.{md,json,csv} + manhours-workflow-ranking.csv — generated
#     by manhours-rollup.py (the 2026-09-25 (bg) direction), which had
#     report/json/csv writers but NO --check mode; the 2026-09-25 (bk) sixth
#     policy-pass sweep (by direction: review everything; implement
#     recommendations) added the mode (in-memory re-derivation of all three
#     artifacts + byte-compare — the same harness class as the two reports
#     above) and wired it here, closing the last generated-analysis surface
#     whose freshness no rule read. The 2026-09-25 (bl) seventh pass promoted
#     the hand-derived ranking CSV — found on disk after (bk), byte-exact
#     re-derivable from the json but written by no tool — into the tool's
#     fourth artifact (--ranking writer + --check arm), the same stray-to-
#     harness promotion class as the (ba) register.
# virtual-gemba-walk.py itself stays unwired BY DOCUMENTED DECISION (the
# methodology-index row: analytical baselines legitimately move with gap-fill
# batches) — Check 80 pins its shipped OUTPUT, not the engine.
C80A_OUT=$(python3 "$REPO_ROOT/07-methodology/role-coverage-gap-analysis.py" --check 2>&1) && C80A_RC=0 || C80A_RC=$?
C80B_OUT=$(python3 "$REPO_ROOT/07-methodology/verify-weak-anchor-demand.py" --check 2>&1) && C80B_RC=0 || C80B_RC=$?
C80C_OUT=$(python3 "$REPO_ROOT/07-methodology/final-semantic-coverage.py" --all --quiet 2>&1) && C80C_RC=0 || C80C_RC=$?
C80D_OUT=$(python3 "$REPO_ROOT/07-methodology/manhours-rollup.py" --check 2>&1) && C80D_RC=0 || C80D_RC=$?
echo "    $C80A_OUT"
echo "    $C80B_OUT"
echo "    $C80C_OUT"
echo "    $C80D_OUT"
if [ $C80A_RC -eq 0 ] && [ $C80B_RC -eq 0 ] && [ $C80C_RC -eq 0 ] && [ $C80D_RC -eq 0 ]; then
    ok "Generated-analysis artifacts byte-current and detector net clean: role-coverage-gap-analysis.md byte-identical to role-coverage-gap-analysis.py --check's re-derivation, weak-anchor-demand-verification.md byte-identical to verify-weak-anchor-demand.py --check's re-derivation (the mode added by this check's wiring — until then every wave verified the shipped report by a manual no-op regeneration + diff), the final-semantic-coverage D1–D4 detector net exits 0 over the full corpus (guard wired into the validator by the 2026-09-23 eighty-second-wave consistency review — the gap report's --check had existed unwired, the verification report had no harness at all, and the detector suite ran manually beside the validator by every wave since the closure), and the manhours roll-up's report/json/csv/ranking artifacts are byte-identical to manhours-rollup.py --check's re-derivation (the mode issued by the 2026-09-25 (bk) sixth policy-pass sweep — the (bg) tool shipped three artifacts whose freshness no rule read; the ranking joined as the fourth artifact by the 2026-09-25 (bl) seventh pass, which found it on disk as an orphan hand-derivation of the json and promoted it into the tool))"
else
    error "Generated-analysis artifact drift / detector flags (run the tools in full for detail):"
    [ $C80A_RC -ne 0 ] && echo "$C80A_OUT" | sed 's/^/    [gap-report] /' | head -25
    [ $C80B_RC -ne 0 ] && echo "$C80B_OUT" | sed 's/^/    [weak-anchor] /' | head -25
    [ $C80C_RC -ne 0 ] && echo "$C80C_OUT" | sed 's/^/    [detectors] /' | head -25
    [ $C80D_RC -ne 0 ] && echo "$C80D_OUT" | sed 's/^/    [manhours] /' | head -25
fi

echo ""
echo "--- Check 81: Production volume-of-record register ---"
# --- Check 81: Production volume-of-record register (generated, cache byte-pinned) ---
# The model company mirrors an actual operator's production EBS (CitiHardware
# PROD); the 2026-09-25 (ba) direction made that estate the volume of record
# ("make ~/access_ebs our model since that's realistic and actual"). The
# register (01-model-company/production-volume-of-record.md) is generated by
# production-volume-of-record.py --refresh from a live read-only query set and
# pinned by the instrument's --check mode (re-render from the committed JSON
# cache + byte-compare). This check runs the mode so the register can never
# drift from its extraction of record. The instrument carries no credentials
# (environment or ACCESS_EBS_HOME notes); --check is fully offline, so the
# validator stays deterministic — refresh cadence is monthly / before any
# calibration decision, by direction.
C81_OUT=$(python3 "$REPO_ROOT/07-methodology/production-volume-of-record.py" --check 2>&1) && C81_RC=0 || C81_RC=$?
echo "    $C81_OUT"
if [ $C81_RC -eq 0 ]; then
    ok "Production volume-of-record register byte-identical to its extraction-of-record cache re-render (the standing CitiHardware-PROD instrument issued by the 2026-09-25 (ba) direction — one-off read-outs became a re-derivable register; §4 is the calibration surface, and no canon figure changes without explicit direction)"
else
