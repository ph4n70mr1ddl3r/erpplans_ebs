...
| 10 | Employees + assignments + EITs | 6,911 headcount (200×29 stores + 600 DC + 511 HQ active; Trade / Account Management's 7 disabled — prepared (x); the IT estate's TPS build squad's 7 deferred — prepared (ad) and the OMO build squad's 7 deferred — prepared (ah), registry CAP-B01) | `HR_PERSON_API`/assignment APIs + EIT loads | W4 — per mapping §2.x |
...
*Document Version: 2.1 | Date: 2026-09-23 | **OMO build squad deferred — prepared (2026-09-23 (ah)): §2 row 10.** The target employee count re-bases 6,918 → 6,911 active (518 → 511 HQ; the OMO build squad's 7 deferred — prepared with every sale completing as a regular POS sale; OM v3.28, TO v3.2). No mapping-row, sequence or load-order change. Prior 2.0 |
