...
6. **Included entitlements ride the base licenses — no separate line:** GL budgets + budgetary
   ...
   Base/Inventory bases (F7), Report Manager inside GL (H18), e-Commerce Gateway inside the
   EBS base (B14 — verify at RFQ), and the procurement/project-procurement ECC dashboards
   inside the adopted ECC (H5).
...
the Financials/AR/AP/CE bases — no lines.*
...
20 AU ⚠ (the claims-processing population was 10).*
...
book value at RFQ (a perpetual true-up, not a re-buy).*
