...
| ERP → Jira Software: enhancement-request sync | Near real-time (on approval) | Approved W616.1 self-service enhancement request → IAP-governed API edge → Jira Software delivery backlog (A6.6 guardrail 1 — the one deliberate split intake; JSM never re-collects what EBS captures transactionally) | Intake-handoff tie-out per W374 triage (approved requests vs synced issues — zero unlinked) |
| Atlassian Cloud → SIEM: organization audit-log stream | Continuous (streaming) | Atlassian Guard organization audit logs (JSM/Jira/Confluence/Bitbucket) → IAP security edge → in-house SIEM (W367) | SIEM ingestion heartbeat; admin-role-change events vs the W1408 quarterly entitlement review |
...
*Document Version: 1.3 | Date: 2026-09-23 | **Seventy-fifth-wave consistency review — the two Atlassian-flow rows join the pattern register.** The A6.6 incorporation (assumptions v4) asserted the W616.1 → Jira enhancement-request sync and the Atlassian Guard → SIEM audit-log stream; the canonical matrix (data-volumes §3 v4.8) now carries them as rows 21–22 and this register mirrors one-for-one per its standing contract — the Jira row rides the IAP API edge with the W374 intake-handoff tie-out, the SIEM row the IAP security edge with the ingestion-heartbeat/W1408 controls; no EBS-native mechanism, SLA or existing-row change. Prior v1.2 | Date: 2026-09-21 | Batch-27 true: the vendor/partner document-exchange channel division named — iSupplier portal (B4) / XML Gateway cXML-XML (W283) / Oracle e-Commerce Gateway classic X12-EDIFACT EDI (fit-gap B14, coverage register EDC-13) — with the owning flows listed; no pattern-register row, SLA or reconciliation change. Prior v1.1 | Date: 2026-09-14 | Two-tier sourcing doctrine enacted: integrated-systems wording (already-built POS/ecommerce/loyalty platforms; in-suite WMS; in-house builds), payroll posting row added to the pattern register (GL_INTERFACE via IAP, E8), government-channel adapters fed by EBS and the payroll build. Prior v1.0 (2026-09-14): initial issue — EBS-native mechanism set, IAP-first rule, per-flow pattern register mirroring the canonical integration matrix, government-channel adapter rule, master-data flow directions, error taxonomy, new-interface gate. Canon anchors: data-volumes-and-integrations §2–§5; sourcing model §7 rule 3 (contract-first); fit-gap LOC/INT rows.*
