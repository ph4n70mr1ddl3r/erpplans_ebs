...
| Companion installation | the CitiHardware R12.2.4 PROD-clone test rig (see [ebs-oratest-verification.md](ebs-oratest-verification.md)) — the capability of record for what the documentation set can execute on the operating company's own installation; this report's two instance-side blocks (VT-2 GL journal import, VT-7 receiving) are GREEN there | OR-2 of the companion report |
