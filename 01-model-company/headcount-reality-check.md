...
> **STATUS — ACTIONED 2026-06-20.** This analysis identified that HQ (315) was understaffed and omitted 6 functions. Per direction to apply the **minimum** increase that resolves the gaps, HQ was rebalanced **315 → 357** and total **6,715 → 6,757** (see `model-company-profile.md` §3.3/§4 and the CHANGELOG entry for 2026-06-20). The comfortable/fully-cushioned range estimated below (~440–470) was **not** adopted; the implemented 357 is the floor that (a) fixes the Executive inconsistency, (b) breaks out the 6 hidden departments explicitly, and (c) relieves only the contradictions the workflows themselves expose (e.g., AP "10 clerks", IT archipelago field support). **2026-06-25 update:** Supply Chain & Logistics was subsequently rebalanced **35 → 40** to stand up a dedicated **S&OP/IBP sub-team** (5) — the single accountable owner of [VS-127](workflows/VS-127-sales-operations-planning-integrated-business-planning/README.md) — closing the §4 "S&OP Lead" missing-role gap and the VS-127 "unowned-as-a-program" finding; HQ is now **357 → 362** and total **6,757 → 6,762** (see `model-company-profile.md` §3.3 "Supply Chain & Logistics Team Structure" and the CHANGELOG entry for 2026-06-25). The analysis below is retained as the gap record. **2026-09-02 update:** the comfortable range (~440–470) estimated below has now been **adopted as the target state** — HQ **469** / total **6,869** — via [`optimal-table-of-organization.md`](optimal-table-of-organization.md) (phased per its §11); the implemented 362 remains the current-state baseline, so this analysis now serves as both the gap record and the sizing authority behind the target bands. **2026-09-03 update:** the hybrid capability-sourcing decision (best-of-breed edges + in-house builds around the unified ERP core; [`07-methodology/capability-sourcing-and-engineering-model.md`](../07-methodology/capability-sourcing-and-engineering-model.md)) extends the IT band **65–80 → 65–130** and the comfortable HQ range **~440–470 → ~440–515** (the agentic-AI extension of the same date adds the final +5 of headroom); the adopted target moves to HQ **511** / total **6,911** with IT = 122 ([`optimal-table-of-organization.md`](optimal-table-of-organization.md) v1.5; [`it-product-operating-model.md`](../07-methodology/it-product-operating-model.md) v3.11 §9). **2026-09-09 update:** [`optimal-table-of-organization.md`](optimal-table-of-organization.md) **v2.1** is now the **official table of organization of record** and defines every one of the 511 HQ roles in its new §5.3 role register (titles drawn from the workflow-RACI canon; v2.1 trues the register's own self-descriptions — 188 rows / 187 distinct titles, the §5.3 span-note row counts, and the §5.2 GL & Consolidation mix cell); the need bands below remain the sizing authority behind its department targets. **2026-09-14 update:** the two-tier sourcing doctrine is enacted — [`optimal-table-of-organization.md`](optimal-table-of-organization.md) **v2.2** (two-tier wording conformance + the IT shape note; register, 511 HQ / 6,911 total and every figure unchanged) and [`it-product-operating-model.md`](../07-methodology/it-product-operating-model.md) **v3.12** §9 (WLI's second vendor seat converted to in-suite WMS/OTE functional-analyst depth; one Platform Product Manager per integrate team; commodity-vendor commercial ownership consolidated in the CIO Office — 122 FTE / 17 teams unchanged); the 65–130 two-tier band and the comfortable HQ range ~440–515 stand. **2026-09-14 promotion:** the optimal TO is **promoted to the actual structure of record** by executive direction — the org recognizes one state (HQ 511 / total 6,911; IT = 122 as the 17-team product-centric department): [`optimal-table-of-organization.md`](optimal-table-of-organization.md) **v2.3** (two-state collapse, §11 recast as the promotion record) and [`it-product-operating-model.md`](../07-methodology/it-product-operating-model.md) **v3.15** (the model as the IT department's structure of record); profile v3.0 re-based (§3.3/§4/§11.1/§13.1; revenue/employee re-derived at each re-base — now ~PHP 8.99M at the 2026-09-18 gap-fill denominator); this gap record's 6,757/33 figures stand as the historical authoring-time record. **2026-09-15 update:** [`optimal-table-of-organization.md`](optimal-table-of-organization.md) **v2.4** states the §5.3 role-resolution contract honestly — the v2.0 note's 'every workflow owner resolves to exactly one row' claim held for only part of the catalog (owner cells 2,507 resolved / 2,920 carrying variant or newer vocabulary of 5,427; 2,040 distinct uncharted owner forms; 10,829 distinct uncharted forms across all RACI surfaces, per the generated role-coverage matrix's watchlist); [`optimal-table-of-organization.md`](optimal-table-of-organization.md) **v2.5** the same day reconciles the vocabulary to the org of record — every recurring uncharted Owner form, every systematic abbreviation, and the deep-tail compound cells adjudicated to register titles, rosters, IT seats, department actors and governance/system/external buckets (495 alias mappings plus 17 malformed Owner cells rewritten in their workflow files), resolving **all 5,427 Owner cells** (owner_uncharted = 0; 5,125 distinct uncharted forms remain across participant/step prose as the governed watchlist), with the census pinned by validate-repo.sh Check 71 — and re-contracts workflow presence at function level (every department has ≥1 catalog-resolved role; specialist/deputy titles may be exercised through their department's broader titles), with the census re-derived and pinned by `07-methodology/validate-repo.sh` Check 71 so the population moves only by conscious re-adjudication; no HC, role, or figure change — 511 / 6,911, IT 122 stand. **2026-09-18 update:** the TO was benchmarked against the operating company's actual table of organization (CitiHardware TOM workbook) and the repo's own virtual-gemba/time-and-motion staffing-claim audit, surfacing four store-embedded workload cells the workflow-derived bands could not see (field-HR coordination, store operations-compliance auditing, installation-services order management, process/document stewardship) — [`optimal-table-of-organization.md`](optimal-table-of-organization.md) **v2.6** fills the gap: IA 9→14, HR 42→55, CS 34→35, Strategy 4→6, **HQ 532 / total 6,932** (§11 Phase 4, +21); profile v3.1 re-based (§3.3/§4/§11.1; revenue/employee re-derived ~PHP 62.3B ÷ 6,932 ≈ PHP 8.99M); store (29/store) and DC (150/DC) models deliberately unchanged — the residual store-side density delta vs the actual operator is a labor-model decision (FTE vs SP/flex), not adopted. **2026-09-21 update:** companion pins re-pointed by the sixty-second-wave consistency review — [`optimal-table-of-organization.md`](optimal-table-of-organization.md) **v2.7** (the §5.3 census-note parenthetical re-pointed to the 5,432-owner-cell canon Check 71 re-pins) and [`it-product-operating-model.md`](../07-methodology/it-product-operating-model.md) **v3.16** (§11's companion-artifacts catalog quote trued to 5,432-WF); no HC, role, register or sizing change — HQ 532 / total 6,932, IT 122 / 17 teams stand. **2026-09-23 update:** companion pins re-pointed by the sixty-sixth-wave consistency review — [`optimal-table-of-organization.md`](optimal-table-of-organization.md) **v2.8** (two procurement sizing-basis cells trued to the calibrated ~12,000 PO-lines/month canon) and [`it-product-operating-model.md`](../07-methodology/it-product-operating-model.md) **v3.17** (companion re-point only; profile **v3.3** trues §6.5's trade-PO band to its §15.1 canon); no HC, role, register or sizing change — HQ 532 / total 6,932, IT 122 / 17 teams stand. **2026-09-23 update (seventy-first wave):** companion pins re-pointed by the seventy-first-wave consistency review — [`it-product-operating-model.md`](../07-methodology/it-product-operating-model.md) **v3.18** (companion re-point only; profile **v3.4** trues §5's POS transaction-mix returns bullet to the production-measured ~0.04% AR-credit-memo canon, the returns-canon cascade across the returns workflows); no HC, role, register or sizing change — HQ 532 / total 6,932, IT 122 / 17 teams stand. **2026-09-23 update (seventy-fifth wave):** companion pins re-pointed by the seventy-fifth-wave consistency review — [`it-product-operating-model.md`](../07-methodology/it-product-operating-model.md) **v3.19** (companion re-point only; the sourcing model **v3.9** names the Atlassian Cloud IT-tooling suite in its §8 vendor-commodity estate per A6.6); no HC, role, register or sizing change — HQ 532 / total 6,932, IT 122 / 17 teams stand. **2026-09-23 update (pickup-only BOPIS mandate):** companion pins re-pointed — [`it-product-operating-model.md`](../07-methodology/it-product-operating-model.md) **v3.20** (companion re-point only; profile **v3.5** re-canons the ecommerce channel as pickup-only BOPIS with POS-tender completion — every ecommerce sale is picked up at the customer-chosen store and completes as a regular POS sale there); no HC, role, register or sizing change — HQ 532 / total 6,932, IT 122 / 17 teams stand. **2026-09-23 update (capability registry):** companion pins re-pointed — [`it-product-operating-model.md`](../07-methodology/it-product-operating-model.md) **v3.21** (companion re-point only; profile **v3.6** defers §8.1 to the new channel-capability-registry — 16 rows, BOPIS the only enabled online sales capability, disabled-prepared posture for delivery/marketplace/COD); no HC, role, register or sizing change — HQ 532 / total 6,932, IT 122 / 17 teams stand. **2026-09-23 update (capability-switchboard workflow, batch 30):** by direction the switchboard itself gains a workflow-level owner — **W5580 Capability Switchboard Operation & Channel Enablement Impact Governance** (PA-113.2, VS-113; confirmed directly Tier 2) — whose step-4 headcount & staffing impact assessment is the no-silent-absorption gate for every future enable/disable flip; corpus 5,432 → **5,433** workflows; [`it-product-operating-model.md`](../07-methodology/it-product-operating-model.md) **v3.22** re-derived (CIO Office 84 → 85; reconciliation 4,937 + 496 = 5,433) and [`optimal-table-of-organization.md`](optimal-table-of-organization.md) **v2.9** (the §5.3 status note's resolution census re-pointed to 5,433 of 5,433 owner cells); no HC, role, register or sizing change — HQ 532 / total 6,932, IT 122 / 17 teams stand. **2026-09-23 update (trade-desk disablement, (x)):** by direction the Trade / Account Management department (7 HQ roles) is **DISABLED — PREPARED** — not staffed, design retained ([`optimal-table-of-organization.md`](optimal-table-of-organization.md) **v3.0** §5.3; registry CAP-B01–B04; [`it-product-operating-model.md`](../07-methodology/it-product-operating-model.md) **v3.23** companion re-point); B2B customers are retail POS customers at any store; the **active** canon re-bases **HQ 525 / total 6,925** (revenue/employee re-derived at the then-active canon; superseded by the (ad) segment below) with the 532/6,932 design standing in the TO of record; IT 122 / 17 teams stand. **2026-09-23 update (seventy-ninth wave):** companion pins re-pointed — [`model-company-profile.md`](model-company-profile.md) **v3.8** (§4's Corporate-HQ-Personnel row re-cut to the two-canon active form — the (x) settlement's §4-table footing residue: the table's rows must foot to 5,800 + 600 + 525 = 6,925), [`07-methodology/technical-guidelines.md`](../07-methodology/technical-guidelines.md) **v3.6** (§2.2's HQ bandwidth-sizing cell re-based to ~525 active HQ staff, ≈473 concurrent users at the ~90% ratio) and the [`licensing BOM`](../02-oracle-ebs/licensing-bom.md) **v2.11** (§2.4's iExpenses filer population re-based to 650 filers ≈ 7,800 reports/yr, licensed band unchanged); [`it-product-operating-model.md`](../07-methodology/it-product-operating-model.md) **v3.24** companion re-point; no HC, role, register or sizing change — HQ 525 active / total 6,925 active (the TO's 532/6,932 design stands), IT 122 / 17 teams stand. **2026-09-23 update (TPS build squad deferral, (ad)):** by direction the IT department's TPS build squad (7 seats) is **deferred — prepared** with the trade desk disabled — its project-side workload (VS-74 job-site delivery, VS-77 material staging/phasing) is dormant, the 122-FTE 17-team design of record is retained, and the deferral is recorded in the capability-sourcing register's amendment schedule ([`capability-sourcing-and-engineering-model.md`](../07-methodology/capability-sourcing-and-engineering-model.md) v3.15 §4; [`it-product-operating-model.md`](../07-methodology/it-product-operating-model.md) v3.25; [`optimal-table-of-organization.md`](optimal-table-of-organization.md) v3.1; [`model-company-profile.md`](model-company-profile.md) v3.9); consumer bulky install/haul-away stays covered by the in-suite Field Service dispatch core and the CCP dispatch experience layer; **IT 122 → 115 active** (16 of 17 teams active); the **active** canon re-bases **HQ 518 / total 6,918** (revenue/employee ~PHP 9.01M ÷ 6,918) with the TO's 532/6,932 design standing. **2026-09-23 update (eighty-third wave):** independent consistency review — the (ad)-cascade's own residue on unguarded live surfaces, each the half-repaired-cell class on a cell a prior cascade had touched or re-pointed beside: the OM §9.2's Industry-benchmark band re-derived at the active canon (100–168 — a 6,715-era derivation carried through three headcount regimes — → ~104–173; [`it-product-operating-model.md`](../07-methodology/it-product-operating-model.md) **v3.26**), the assumptions register's A6.6 seat-avoidance note re-based ~6,925 → ~6,918-seat (v10), the channel-capability registry's rule-7 headcount parenthetical recut to the two-event lineage and its CAP-B01 checklist endpoints re-based 518 → 525 with the 525 → 532 hop pinned to the sourcing register's re-enablement trigger (registry v1.4), and the sourcing model's §13 OM row clause trued to the (ad) canon (sourcing v3.16); no HC, role, register or sizing change — HQ 518 / total 6,918 active, the 122-FTE 17-team design retained ([`optimal-table-of-organization.md`](optimal-table-of-organization.md) v3.1), IT 115 active of the 122-design stand. **2026-09-23 update (eighty-fourth wave):** independent consistency review — the (x)/(ad) estate's own navigation-surface residue: the profile §14.1's Trade & Project Services Platform row re-cut 'Live — Built In-House (TPS)' → **Deferred — prepared** with the Field-Service-dispatch seam clause ([`model-company-profile.md`](model-company-profile.md) **v3.10** — the executive-summary's identical row the (ad) pass itself re-cut), the chain-level intro capability-state claims annotated per the registry's state discipline (root README + executive-summary: the retired 'all its capabilities and systems fully enabled' form), and the AI-first guide's §13 OM navigation row recut to the two-state form (16 teams / 115 FTE active of the 122-design; guide v1.18); [`it-product-operating-model.md`](../07-methodology/it-product-operating-model.md) **v3.27** companion re-point; no HC, role, register or sizing change — HQ 518 / total 6,918 active, the 122-FTE 17-team design retained, IT 115 active of the 122-design stand. **2026-09-23 update (OMO build-squad deferral, (ah)):** by direction the IT department's OMO build squad (7 seats) is **deferred — prepared** — with BOPIS the only enabled online fulfillment option (registry CAP-F01) every sale completes as a regular POS sale at the customer-selected store and order routing is deterministic, the multi-source orchestration workload (VS-60) being the disabled—prepared fulfillment estate; the 122-FTE 17-team design of record is retained and the deferral is recorded in the capability-sourcing register's amendment schedule ([`capability-sourcing-and-engineering-model.md`](../07-methodology/capability-sourcing-and-engineering-model.md) v3.18 §4; [`it-product-operating-model.md`](../07-methodology/it-product-operating-model.md) v3.31; [`optimal-table-of-organization.md`](optimal-table-of-organization.md) v3.2; [`model-company-profile.md`](model-company-profile.md) v3.11); the live BOPIS path rides the already-built ecommerce platform's capability-configuration service and in-suite order flows; **IT 115 → 108 active** (15 of 17 teams active); the **active** canon re-bases **HQ 511 / total 6,911** (revenue/employee ~PHP 9.01M ÷ 6,911) with the TO's 532/6,932 design standing. **2026-09-23 update (role-level workflow-anchoring contract):** by direction every role must be explicitly present in the workflow catalog to be measurable — a role with no Owner/Participants/Step presence generates no demand signal, no cycle time and no handoff count, so headcount and structure optimization cannot be scientific for it — and the TO's §5.3 workflow-presence contract is tightened accordingly from function level (this analysis's own v2.4/v2.5-era standing) to **role level** ([`optimal-table-of-organization.md`](optimal-table-of-organization.md) **v3.5**): every chartered role (each register row, IT product-model seat, §7.2/§7.3 roster role) must carry ≥1 explicit RACI anchor. The anchoring census — **259 chartered / 259 anchored / 0 zero-anchor / 65 weakly anchored** (the initial 56-role zero-anchor worklist was cleared the same day by the batch-31 remediation, `07-methodology/fix-role-anchor-worklist.py`: 42 register roles, both IAP seats and all nine §7.3 DC-roster roles anchored into their mandate workflows, picker/packer-class corpus vocabulary promoted to the chartered titles it denotes, and three §7.2 staffing-note fragments re-scoped out of the census; the 65 weakly anchored roles form the demand-verification queue) — is re-derived every run and pinned by `validate-repo.sh` Check 71, and the zero-anchor worklist (permanently empty unless a future charter mints an unanchored role — which then fails Check 71) plus the weak-anchor demand-verification watchlist are emitted in the generated role-coverage matrix's Role-Anchoring Contract section — this gap record's discipline made continuous and per-role; no headcount or structure decision may touch an unanchored or demand-unverified role (anchor it or consciously re-scope the charter, then verify per-role annual demand via `virtual-gemba-walk.py` against chartered capacity). No HC, role, register or sizing change — HQ 532 / total 6,932 design, 511 active, IT 122 / 17 teams stand. **2026-09-25 update (weak-anchor overload dispositions):** the five demand-verified OVERLOAD rows the anchoring contract's instrument surfaced (Senior LP Analytics Analyst, Assistant DC Manager — Inbound, Quality & Workforce Analyst, Special Handling Lead, Privacy Officer exercised-through) are dispositioned in the re-scope lever — the mis-attributed workflow Role cells re-pointed to the executing seats and the time-and-motion engine's per-occurrence/period-bridge grammar repaired ([`optimal-table-of-organization.md`](optimal-table-of-organization.md) **v3.6** §5.3 decision record) — with **no headcount, register-row, reporting-line or sizing change**; HQ 532 / total 6,932 design, 511 active, IT 122 / 17 teams stand; the verification queue now reads OVERLOAD 0.
