...
| # | Activity | Role (R) | Role (A) | Duration |
|---|---|---|---|---|
| 11 | LPO documents confirmed loss: investigation findings, supporting evidence (CCTV footage, transaction records, witness statements); for theft, files police report with local PNP station | LPO | Internal Audit | 1 hour/case |
| 12 | LPO submits inventory write-off request: SKU, quantity, value at WAC, root cause classification (internal theft, external shoplifting, vendor fraud, unexplained loss) | LPO | Internal Audit | 15 min/case |
| 13 | Approval per tier: (a) total loss ≤ PHP 10,000: Store Manager, (b) PHP 10,001–50,000: Regional Manager + Internal Audit, (c) PHP 50,001–500,000: Controller + Internal Audit, (d) > PHP 500,000: CFO + CEO | Approver | Approver | 15 min/case |
| 14 | System posts write-off: Dr. Inventory Loss / Cr. Inventory; removes items from inventory register; records loss in shrinkage tracking for store KPI | System | — | Automated |
| 15 | If loss is insured (e.g., large robbery, transit theft): LPO files insurance claim per W3.6a insurance claim process; Finance posts recovery upon settlement | LPO / Finance | Controller | Per W3.6a |
| 16 | Quarterly: LPO includes confirmed theft and write-off totals in shrinkage report (W37.7); feeds into store KPI scoring and loss prevention action plans | LPO | Internal Audit | Part of existing reporting |
...
### Automation Opportunity
- System auto-alert of "COO monitors PAGASA bulletins and NDRRMC advisories" (replaces manual Step 1).
- System auto-suspend of "suspends BOPIS and delivery orders for closed stores/DCs" (Step 10, already system-executed).
- System auto-reconcile of "process any offline POS transactions from pre-closure; reverse pending ecommerce orders for closed stores" (Step 21, already system-executed).
...
*Counts below reflect the **promoted HQ staffing of record, gap-filled 2026-09-18** (profile §3.3; the design register's 532 — the active org is **HQ 511** since the 2026-09-23 (x) trade-desk disablement stood the 7-role Trade / Account Management department down as DISABLED — PREPARED, registry CAP-B01, the 2026-09-23 (ad) direction deferred the IT estate's TPS build squad (7) — prepared, sourcing register §4, and the 2026-09-23 (ah) direction deferred the OMO build squad (7) — prepared with every sale completing as a regular POS sale; the cells below carry the design register's values, which run as designed capacity). The 2026-06-20/25 rebalance — which first increased Finance, IT, HR, and Legal from the prior lean baseline to resolve the capacity contradictions flagged in their own validation notes (e.g., AP "10 clerks" cannot fit a 35-person team; IT archipelago field support flagged inadequate) and first enumerated the six previously un-enumerated functions (HSE, Quality, Facilities & Real Estate, Sustainability/ESG, Strategy, Trade Account Management) — is retained in the profile's §3.3 history note and the official TO's §5.1 reference column; the department cells below carry the promoted register's values as re-based by the 2026-09-18 actual-org gap-fill (IA 14, HR 55, CS 35, Strategy 6). Store (5,800) and DC (600) counts are unchanged.*
...
| **HR & Payroll** | HR Head, Recruitment, Payroll, Training Officer, HR Assistants, Store HR Coordinators | ~55 | W10, W15, W51, W72, W79 (2307 distribution), W183 (trainee admin) | ✅ 2–3 payroll officers + 2 recruiters + 1 Training Officer handle the volume; employee performance management absorbed by ~230 managers; the 2026-09-18 actual-org gap-fill adds 13 Store HR Coordinators (one per district) for store-embedded people administration (timekeeping exceptions, 2307/ETEE distribution via VS-79, DTS/TESDA trainee administration via VS-183, ER first line, DOLE inspection readiness) — workload the actual operating company staffs ~1 store HR head per store |
...
| **IT** | The 17-team product-centric operating model (9 domain + 7 platform teams + CIO Office; OM §3/§5) | ~108 active (122 design; the TPS build squad's 7 deferred — prepared, (ad), and the OMO build squad's 7 deferred — prepared, (ah)) | W16 (store setups), W35 (reporting), W48 (helpdesk & IT ops) | ✅ The IT department runs as the product-centric operating model (108 active FTE of the 122-design; OM v3.31): helpdesk L2 analysts (2, with outsourced L1) handle ~800–1,200 tickets/month; 2–3 per store setup + BAU support; management reporting (W35) runs on the DP semantic layer with data & reporting analysts in the domain teams (OM §5.2). The archipelago field-support gap flagged by the headcount reality check under the pre-promotion 50-FTE baseline is resolved by Field & End-User Services (FS): 5 regional field technicians (one per major island region: Mindanao, Visayas, South Luzon, Metro Manila, North/Central Luzon), outsourced L1 + 2 L2, ITAM, hardware lifecycle for the 600-terminal POS estate, DC swap stock, and store-visit SLAs (OM §5.3) |
| **HSE, Quality, Facilities, ESG, Strategy, Trade Acct Mgmt, Regional LP** | Safety officers/nurse, incoming inspection & metrology, facilities & lease admin, ESG reporting, corporate planning + BPM/IMS, trade program, LP leadership + field officers | ~74 | W140–W141 (HSE), W31 (quality/incoming inspection), W47 (facilities), W25.x (ESG), W33.x (strategy), W43.x (trade program), VS-23 (loss prevention) | ✅ The six previously un-enumerated functions (profile §3.3: HSE 13, Quality 5, Facilities & Real Estate 12, Sustainability/ESG 4, Strategy 6, Trade/Account Management 7) plus the 27-person Regional LP department (Director, 20 regional LP officers, investigations, analytics) — completing the table reconciliation to the HQ total (first closed at the 2026-06-25 rebalance's 362; re-cut to the promoted 2026-09-14 register; re-cut to the 2026-09-18 gap-filled register; re-cut to the 2026-09-23 (x) disablement — active HQ 525, the design register's 532 retained; re-cut to the 2026-09-23 (ad)/(ah) build-squad deferrals — active HQ 511, the design register's 532 retained) |
| **Other** | Legal, Internal Audit (+ operations-compliance field auditors), DPO, Regulatory Officer, Customer Service (call center + services manager), Executive | ~77 | W41 (complaints), W42 (audit observation), W53 (data privacy breach, DSAR lifecycle, data portability/erasure), W54 (LGU permits), W54A (BIR CAS registration), W62 (vendor contracts), W12 (installation/repair service orders) | ✅ Support functions; call center handles multi-channel complaint intake; Legal expanded to include DPO (W53 with full DSAR lifecycle) and Regulatory Officer (W54 LGU + W54A BIR CAS); total Legal & Compliance is 20 per profile §3.3 (incl. the DPO and Regulatory Officer); IA carries the 5-person operations-compliance cell (2026-09-18 actual-org gap-fill) for the store-audit cycle; CS carries the Services Manager seat for the VS-12 service-order lifecycle |
...
### Automation Opportunity
- System auto-file of "upon shipment departure" (replaces manual Step 1).
...
- Oracle Quality (fit-gap C12): root cause analysis, CAPA tracking (per W110)
...
### System Touchpoints
- Data Processing System Inventory register on the in-house Audit & GRC platform (BUILD — fit-gap H11) (W1203.1)
...
- Employee privacy training tracking in OLA (W1203.7)
