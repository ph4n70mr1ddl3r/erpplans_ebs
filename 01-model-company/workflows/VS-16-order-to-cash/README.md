# VS-16: Order-to-Cash

> **Finance** · [Value Stream Index](../value-stream-index.md)

---

## Overview

Order-to-cash: credit application & scoring, AR & collections, and customer payment & settlement for ~5,200 trade accounts.

## Why it matters

~40% of revenue is B2B credit; credit-risk discipline, collections, and payment integrity govern DSO, bad debt, and working capital. PDC and CWT (2307) are PH-specific.

## Owner & participants

- **Owner**: Credit Manager / Controller

## Process Areas

| PA | Name | Workflows |
|---|---|---|
| [PA-16.1](PA-16.1-credit-application-and-scoring.md) | Credit Application & Scoring | 12 |
| [PA-16.2](PA-16.2-ar-and-collections.md) | AR & Collections | 11 |
| [PA-16.3](PA-16.3-customer-payment-and-settlement.md) | Customer Payment & Settlement | 9 |
| | **Total** | **32** |

## Key dependencies

VS-11 (trade), VS-68 (credit risk), VS-17 (GL), VS-79 (tax), VS-107 (key accounts)

## Key controls

CTL-05 (credit limit), W1380 PDC, W1381 bounced-check, W475 CWT/2307, W81 bad-debt provisioning

---

*Back to [Value Stream Index](../value-stream-index.md)*
