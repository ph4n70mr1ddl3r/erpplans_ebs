...
| **Volume** | ~2 lines per receipt on average (the ~12,000 PO-lines/month all-types canon across the ~6,000 goods-receipts/month canon — bulk POs arrive as multiple partial truck receipts, per data-volumes §1.1/§1.2) |
| **Owner** | DC Receiving Supervisor |
| **Participants** | Receiving Clerk, Quality Checker, Putaway Staff, DC Supervisor, Buyer (if discrepancy), Special Handling Lead, Discrepancy Analysts |
...
| 6 | If discrepancy (shortage, damage, wrong item): flag in system; notify Buyer | Receiving Clerk, Discrepancy Analysts | DC Supervisor | 5 min |
...
| 9a | Special-handling receipts (lumber/long-length, breakbulk, hazmat): crew moves goods to the designated handling area or bin under the Special Handling Lead's direction; scan-confirm in system | Special Handling Lead | DC Supervisor | 30–45 min/occurrence |
...
| **Volume** | ~1,200 merchandise receipts/month + ~80–240 blanket releases + ~100–150 import containers (the ~400–600 TEUs/month §7.1 canon at ~1 TEU/container across 4 DCs, per W1271) + ~30–50 non-merchandise receipts per DC per month |
| **Owner** | DC Receiving Supervisor |
...
| # | Activity | Role (R) | Role (A) | Duration |
|---|---|---|---|---|
| 1 | **Gate-In Registration**: Gate Guard inspects seal integrity, container physical state, and chassis lease records; scan-registers container number in Oracle WMS Yard Management (fit-gap C6); records gate-in timestamp | Gate Guard | Yard Superintendent | 10 min |
...
| **Frequency** | ~6,000 DC goods receipts/month across 4 DCs (the goods-receipt canon); portal-enabled vendors account for ~80% of receipt volume, with ASN pre-notification riding the PO spine — ~700–750 ASNs/month, ~60% of the ~1,200 merchandise POs (per W1006; ~40% of vendors by count lack access — see pain points) |
| **Volume** | ~700–750 ASN submissions/month (1 ASN per PO per W1006 — the ASN spine is the ~1,200-merchandise-PO universe, not the ~6,000-receipt universe; a PO delivered across multiple partial truck receipts carries one ASN); ~15–20% have discrepancies requiring exception handling = ~105–150 exceptions/month |
...
| 5 | DC Inbound Supervisor reviews exception: determines resolution — (a) accept shipment with PO amendment by Buyer, (b) accept partial shipment (only matched lines), (c) reject entire shipment, or (d) hold shipment pending vendor clarification | DC Inbound Supervisor | DC Manager | 10–20 min/occurrence |
...
| 7 | For resolution (d): Supervisor contacts vendor via portal messaging; vendor provides corrected ASN or explanation; system matches corrected ASN to PO | DC Inbound Supervisor / Vendor | — | 15–30 min/occurrence (vendor response window 1–24 hours elapsed) |
...
- Exception handling (15–20%): 10–20 min × ~105–150 exceptions/month = ~18–50 hours/month across 4 DCs = ~4–13 hours/DC/month
- Buyer involvement for PO amendments: ~15 min × ~25–35 cases/month = ~6–9 hours/month across all buyers
- Weekly report: 1 hour/week × 4 DCs = 4 hours/week
- Total: ~40–75 hours/month across all participants

### Staffing Implication
- DC Inbound Supervisor: ~4–13 hours/DC/month on ASN exception handling; this is a core supervisory function absorbed within existing DC operations headcount
- Buyer: ~6–9 hours/month total across all buyers for PO amendments triggered by ASN mismatches; <1 hour/buyer/month across the 10-buyer team, absorbed within buying duties
