...
| **Frequency** | 2–3 physical deliveries per store per week; ~50,000 replenishment orders/month total (production-calibrated per data-volumes §1.1; each delivery consolidates that store's ~20–30 transfer orders since the previous drop) |
| **Volume** | ~1,700 orders/day across all DCs (~415–425 per DC/day); avg ~3 lines per order (the production-measured order profile, per data-volumes §1.1) |
...
- **2–3 Supply Planners** (in HQ): ~1,700 orders/day to review (the production-calibrated ~50,000/month canon — auto-generated; the planner queue is the ~2,500–4,000/month exception stream per PA-02.1). At 2–3 hours/day for review + 1 hour for wave management = 3–4 hours/day. 2–3 planners share this plus demand forecasting. Reasonable within the 40-person Supply Chain & Logistics team (per profile §3.3).
- **Per DC**: 15–20 Pickers, 8–10 Packers, 4–6 Loading Crew (working in shifts to handle ~415–425 orders/day with ~3 lines each ≈ ~1,250 pick lines/DC/day). Fits within ~150 DC headcount.
...
### Automation Opportunity
- System auto-generate of "generate zone-based count sheets per location with expected system quantities (hidden for blind count)" (replaces manual Step 2).
- System auto-flag of "System generates variance report per zone" (Step 9, already system-executed).
- System auto-post of "System posts approved inventory adjustments" (Step 14, already system-executed).
