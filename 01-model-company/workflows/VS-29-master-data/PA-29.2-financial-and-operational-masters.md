...
| 4 | **System Setup & Mapping**: IT ERP Administrator configures the new cost center as an accounting-flexfield cost-center segment value in the CoA, assigns it to its parent roll-up group for financial-statement reporting, sets cross-validation and security rules, and maps it to the relevant financial statement roll-ups. | IT ERP Administrator | Corporate Controller | 30 min |
...
| 2 | **New cost/profit center creation**: For new stores (W16), new departments, or organizational changes — FP&A Manager submits creation request to IT Finance Lead: center code, description, parent hierarchy, entity assignment, responsible manager, valid-from date; IT Finance Lead creates the value in the accounting flexfield — cost-center segment under the department roll-up group for HQ cost centers, the store hierarchy's profit-center segment for stores — validating the company × cost-center cross-validation rule per entity; Finance Controller approves; system propagates to all downstream reporting modules | FP&A Manager / IT Finance Lead | Finance Controller | 30 min/center |
