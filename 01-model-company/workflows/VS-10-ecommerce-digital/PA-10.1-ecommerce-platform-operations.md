...
>
> **Pickup-only checkout (2026-09-23 (d)):** checkout mandates pickup-store selection (store-level ATP displayed); payment is **prepaid at checkout** (CC/GCash/Maya/bank transfer via the gateway) or **pay-at-pickup** (reservation-only). Every order — prepaid or not — completes as a regular POS sale at the chosen store (W11 steps 9–11): the Online-Prepaid tender line on the POS sale reconciles 1:1 to the gateway settlement (W267, CTL-47). COD retires with the dormant delivery estate; the delivery-failure exception scenarios below remain documented for BCP reactivation only. **Capability states (this platform renders only the ENABLED set — web, app, BOPIS, prepaid, pay-at-pickup) are governed by the [Online Channel & Capability Registry](../../channel-capability-registry.md):** disabled capabilities are hidden at checkout, never grayed; their order-intake endpoints reject with a typed capability-disabled response.
...
| 7 | **Delivery — customer reports wrong item**: (a) Customer contacts Ecommerce Customer Support (chat, email, phone) reporting wrong item delivered; (b) CSR verifies: compares order contents vs. delivery confirmation photo; (c) if carrier error (wrong package delivered): CSR initiates carrier claim (W19.12b) and reships correct item from DC; (d) if DC pick error (DC picked wrong SKU): DC Dispatch investigates pick process; correct item reshipped; wrong item scheduled for carrier pickup (reverse logistics per W19.12a); (e) system creates exception order with priority flag for reshipment | CSR / DC Dispatch, Ecommerce Support Specialist | DC Supervisor | 15 min/case |
...
| **Frequency** | ~500–1,000 returned items/month (the W1623 canon: ~250–350 ecommerce return requests/month ≈ 0.6–0.8% of ~42,900 orders, ~2–3 items per return) |
| **Volume** | ~750 items/month average; peaks post-holiday and post-promo (+40%) |
...
### Time Estimate
~5–12 minutes per item depending on complexity. At ~500–1,000 items/month = ~40–200 staff-hours/month. Requires 1–2 Returns Inspectors at DC (absorbed within DC headcount of 150/DC).

### Staffing Implication
- Returns processing requires ~1–2 Returns Inspectors at the primary DC (per the Time Estimate derivation); absorbed within DC headcount.
...
| **Participants** | Customer Experience Representative, Category Manager, Vendor Manager, Ecommerce Marketing Specialist |
...
| 6 | CX Representative drafts public vendor response for applicable reviews; response posted alongside customer review on product page | CX Rep, Ecommerce Marketing Specialist | Digital Content Manager | 10 min/response |
...
- Receivables Revenue Management (W487 — fit-gap A11) for deferred installation revenue
...
| **Frequency** | Continuous; ~42,900 ecommerce orders/month + ~50,000 store replenishment orders/month (production-calibrated per data-volumes §1.1) + ~3,500 AR invoices/month competing for the same inventory pool |
