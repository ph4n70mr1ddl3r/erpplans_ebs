...
>
> **Pickup-only mandate (2026-09-23 (d)):** by executive direction **every ecommerce sale is a BOPIS order** — checkout offers store pickup only, the customer selects the pickup store, and the sale **completes as a regular POS sale at that store** (cashier order recall on a POS terminal, Online-Prepaid tender for any prepaid amount plus balance tender, BIR receipt printed by POS, revenue/12% VAT and inventory deduction through the standard POS chain — W11 steps 9–11; PA-08.1). The delivery fulfillment estate below (W19, W19B, W210, W246, W592, W829, W899, W1013, W1155, W1195, W1196, W1234) is **dormant**: retained as documented capability for BCP reactivation (BCP-008 pandemic mode, VS-69 typhoon recovery) with its volume documentation held as designed capacity, not live load. Live ecommerce volume is ~42,900 orders/month, 100% BOPIS. **Capability states are governed by the [Online Channel & Capability Registry](../../channel-capability-registry.md)** — checkout renders only the ENABLED set; the dormant estate's rows are DISABLED — PREPARED with enable checklists.
...
| Field | Detail |
|---|---|
| **Trigger** | Customer places BOPIS order on website/app (the only ecommerce checkout path) |
| **Frequency** | ~42,900 BOPIS orders/month; ~1,430/day (100% of ecommerce orders under the pickup-only mandate) |
...
|---|---|---|---|---|
| 1 | Customer places order on website/app; selects pickup store (store-level ATP displayed); pays online (prepaid: CC/GCash/Maya/bank transfer via gateway) or chooses pay-at-pickup (reservation-only) | Customer | — | — |
| 2 | System routes order to selected store; creates store pick list; locks the order price; pushes the order record (POS recall payload) to the store | System | — | Automated |
...
| 6 | Stock Associate stages picked items at Customer Service counter or smart locker (W247) | Stock Associate | CSR | 2 min |
...
| 9 | CSR verifies ID; recalls the order on a POS terminal (order number/QR scan) — POS rings all lines at the order-locked price like any walk-in basket | CSR | Store Manager | 2–3 min |
| 10 | **POS tender completes the sale**: prepaid amount auto-applies as the **Online-Prepaid** tender type (systemic tender via order recall); any balance tendered normally (cash/card/e-wallet); POS prints the BIR receipt; the sale posts through the standard POS chain (W525 streaming → W522 review → W537/541 settlement); inventory deducts via the POS sale | CSR / System | Store Manager | 2–3 min |
| 11 | POS tender confirmation closes the ecommerce order ("Completed"); fulfillment status pushed back to the platform; loyalty/returns ride the POS receipt like any store sale | System | — | Automated |
| 12 | If not picked up within 5 days: auto-cancel; prepaid orders refunded to original payment method per W101/W1442; reservation-only orders simply release the reservation; items returned to shelf | System | CSR | Automated (return: 5 min) |
...
- **POS-tender completion (W11.10) — the sale is a regular POS sale**: cashier order recall on the store terminal, order-locked pricing, **Online-Prepaid** tender type for the prepaid amount plus normal tender for any balance, BIR receipt printed by POS; the sale streams/posts through the standard POS chain (W525/W522, W537/541 settlement) — no separate BOPIS transaction type exists; the POS tender confirmation is the event that closes the online order (W11.11)
- Auto-cancel and refund after hold period (W11.12; W1442)
- Ecommerce IC settlement for BOPIS (pickup-only canon): all BOPIS revenue is recognized by Depot Inc. through the POS sale at the pickup store — indistinguishable in the sales ledger from walk-in revenue; Digital Commerce Inc. remits collected prepaid payments to Depot Inc. monthly per W14 ecommerce payment collection IC flow (net of gateway fees) and charges the per-order fulfillment fee per W1417; for pay-at-pickup orders no IC payment movement arises (tender settles through the store's normal POS settlement W537/541); all IC settlements processed monthly via W14
- Ecommerce VAT for BOPIS (pickup-only canon): BOPIS VAT is ordinary Depot Inc. POS output VAT — 12%, appearing on the POS BIR receipt and inside the store's 2550M filing exactly like walk-in sales; there is no separate ecommerce VAT stream; Digital Commerce Inc. recognizes no VAT on payment collection (remitted to Depot Inc.)
...
- **Customer wait time at pickup counter**: under the pickup-only mandate the counter handles ~1,430 handoffs/day chain-wide, each adding POS tender time (~4–6 min recall + tender + receipt) on top of ID verification; multiple simultaneous arrivals queue at a Customer Service counter staffed by a single CSR — wait times exceeding 10 minutes negatively impact the BOPIS value proposition; smart-locker auto-completion (W247) and W526 queue management are the pressure valves
...
- CSR ID check and order recall on POS: 2–3 min
- POS tender (Online-Prepaid + balance), BIR receipt, sale posting: 2–3 min
- Order completion (POS confirmation closes the online order): automated (instant)
...
- **1 CSR per store**: 1,430 BOPIS orders/day ÷ 200 stores = ~7 BOPIS orders/store/day. At ~5 min handoff per order (ID + POS recall + tender + receipt), that's ~35 min/day of counter time — the CSR also handles returns and special orders, so 1 per store holds, with W247 smart lockers absorbing peak-hour handoffs (locker release auto-tenders the prepaid sale unattended).
- **Stock Associates absorb picking**: ~7 BOPIS picks/day at 5–15 min each (~1–1.75 hr/store/day) is a manageable addition to the 4 stock associates already doing replenishment and cycle counts; pick volume scales with the pickup-only mandate's full ~42,900 orders/month.
...
### Staffing Implication
- **Per DC**: Home delivery adds 17,200 orders/month ÷ 30 days ÷ 4 DCs = **~143 orders/DC/day**. At ~15 min pick+pack per order, that's **~36 hours/DC/day** of additional DC labor. This requires 3–4 dedicated pickers/packers per DC for home delivery, within the existing ~150 DC headcount (total DC pick/pack team of 25–30 handles both store replenishment at ~415–425 orders/day ≈ ~1,250 pick lines/DC/day at the production-measured ~3 lines/order and ~143 home delivery orders/day in shifts). Home delivery orders are typically smaller (3–4 items) and packed individually, while each store-delivery drop consolidates a store's ~20–30 transfer orders (~3 lines each, per data-volumes §1.1) into one bulk shipment — different skills and pacing.
...
| **Frequency** | ~0.3–0.6% of home delivery orders (the courier-pickup leg of the ecommerce returns canon: ≈ 250–350 total return requests/month per W1623, less the ≈ 200–250 store drop-offs per W12B) |
| **Volume** | ~50–100 returns/month at steady state (the courier-pickup remainder of the W1623 canon; peaks post-holiday and post-promo) |
...
### Staffing Implication
- **CSR (return validation and booking)**: ~50–100 returns/month × 20 min per return (validation + booking) = ~17–33 hours/month; distributed across the 30-person Ecommerce Customer Support team = ~0.5–1 hours/agent/month; largely absorbed within existing call center capacity, with post-promotional spikes requiring surge scheduling
- **DC Receiving Team (QC inspection)**: ~50–100 returns/month × 10 min QC inspection per item = ~8–17 hours/month across 4 DCs = ~2–4 hours/DC/month; absorbed by existing DC Receiving Team as part of standard inbound processing, though post-promotional return spikes may require temporary overtime
- **Finance (AR) refund authorization**: ~50–100 refunds/month × 10 min per authorization = ~8–17 hours/month; absorbed by existing AR team within W101 refund processing duties
- **No incremental headcount at the canon volumes**; if courier-pickup volumes grow beyond ~250/month (quality issues or marketplace expansion), a dedicated Returns Processing Clerk at each high-volume DC should be considered
...
| **Volume** | ~7 BOPIS pickups per store/day (~42,900 BOPIS orders/month per profile §8.5) |
...
| 4 | **Customer Arrival**: Customer arrives at the store. (a) For Smart Locker: Scans the QR/enters the PIN at the locker screen; the locker door pops open; the locker release event auto-tenders the sale as an **Online-Prepaid** POS transaction on the store's designated BOPIS terminal (BIR-accredited kiosk/counter printer issues the receipt for collection). (b) For Pickup Counter: Customer checks in on a tablet queueing screen; Pro Desk is alerted; the CSR executes W11 steps 9–10 (ID verification + POS tender). | Customer / System / CSR | Store Mgr | 2 min (locker) / 4–6 min (counter) |
| 5 | **Reconciliation**: The POS tender confirmation (counter handoff per W11 step 11, or the locker auto-tender) triggers the real-time order completion status in the ERP. | System / Associate | Store Mgr | Automated |
...
- POS integration — auto-tender of **Online-Prepaid** sales for locker handoffs (unattended BIR receipt issuance at the BIR-accredited kiosk printer); counter handoffs tender per W11 steps 9–10; the POS tender confirmation closes the order
...
| **Volume** | ~42,900 ecommerce orders/month (~1,430/day), 100% BOPIS under the pickup-only mandate (the dormant delivery origins — home delivery, ship-from-store, DSV — contribute zero live volume; their SLA definitions are retained as designed capacity for BCP reactivation); SLA metrics tracked per order per fulfillment origin |
...
| **Frequency** | **Dormant** under the pickup-only mandate (2026-09-23 (d)): the live order book is 100% BOPIS (~42,900 orders/month tracked to pickup readiness per W11 — no carrier tracking/POD); the tracking and POD mechanics below are retained as documented capability for BCP reactivation of the delivery estate (the W19 canon — ~17,200 home delivery orders/month — is the dormant design basis) |
...
| **Frequency** | ~200–250 ecommerce store drop-offs/month (~0.5–0.6% of ~42,900 ecommerce orders — the store drop-off subset of the ≈ 250–350 total ecommerce return requests per the W1623 canon; ≈ 1–1.25 drop-offs/store/month) |
| **Volume** | ~1 in-store ecommerce return drop-off per store per month (~1–1.25 at the band edges); ~200–250 chain-wide/month (the W12B canon; the courier-pickup remainder ≈ 50–100/month rides W215) |
...
- Monthly per store: ~1 return × 15–25 min = ~15–25 min/month per store
- Monthly chain-wide: ~200–250 returns × 15–25 min = ~50–104 hours/month
- Annual: ~600–1,250 hours/year

### Staffing Implication
- **CSR**: ~15–25 min/month on ecommerce returns per store — well under 1% of the CSR's time (1 CSR per store, ~170 working hours/month); fully absorbed within existing CSR duties (BOPIS pickup, special orders, complaints), post-sale return spikes included.
- **No permanent incremental headcount.**
...
| **Frequency** | ~42,900 BOPIS orders/month (~1,430/day across 200 stores); SLA monitoring continuous; exception escalation as needed |
| **Volume** | ~1,430 BOPIS orders/day; SLA target: 95%+ picked within 4 hours; expected SLA breach rate: 5–8% (~72–114 orders/day needing exception handling) |
...
| **Frequency** | ~8–12% no-show rate = ~3,400–5,100 orders/month across all 200 stores; daily auto-cancel processing; weekly no-show rate review |
| **Volume** | ~42,900 BOPIS orders/month; ~3,400–5,100 no-shows/month; each no-show triggers auto-cancel, refund (prepaid orders), and restock |
| **Owner** | E-Commerce Operations Manager |
| **Participants** | E-Commerce Ops Manager, Store Customer Service Rep, Finance/AP (refund processing), Stock Associate (restocking) |

### Background

BuildRight Depot's BOPIS (Buy Online, Pick Up In Store) service processes ~42,900 orders/month across 200 stores. The service level agreement is a 4-hour pick time (per W11) and a 5-day customer hold period. Not all customers who place BOPIS orders actually show up to pick them up — the no-show rate is estimated at 8–12%, resulting in ~3,400–5,100 unpicked orders/month. These unpicked orders create several problems: (a) reserved inventory is held in the staging area and unavailable for sale to walk-in customers, (b) the staging area consumes limited backroom space, (c) prepaid orders must be refunded (tying up finance team time), (d) the item must be restocked to the sales floor. This workflow manages the end-to-end no-show process: proactive customer reminders, auto-cancel processing at hold period expiry, refund initiation, and restocking.
...
| 2 | **Daily auto-cancel processing**: At end of Day 5 (hold period expiry): (a) system auto-cancels all unpicked BOPIS orders, (b) system initiates refund to original payment method for prepaid orders (credit card, e-wallet, GCash; reservation-only pay-at-pickup orders release the reservation with no payment movement — the POS tender that would have completed the sale never occurred), (c) system generates daily auto-cancel report: order number, SKU, store, value, cancel reason ("hold period expired"), (d) system releases inventory reservation — items become available for walk-in sale | System (automated) | E-Commerce Ops Manager | Automated (30 min/day exception review) |
