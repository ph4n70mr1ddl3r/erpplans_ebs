...
7. **The same state discipline governs the B2B capability estate (§3.4).** Trade / Account Management — the company's business-to-business selling capability — is governed by the same disable-not-delete pattern: disabled capabilities keep their full prepared design (the 7-role department in the TO §5.3 register, the VS-11/VS-43/VS-107/VS-68 process areas, requirements and controls), the corpus never loses a prepared design, and state changes ride **W5580** with same-change registry-row updates. Headcount effects run through the workflow's own step-4 no-silent-absorption assessment: the disablement re-bases the active headcount canon (HQ 532 → 525 active; total 6,932 → 6,925) with the design retained.
